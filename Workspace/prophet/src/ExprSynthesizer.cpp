...
public:
    BasicTester(BenchProgram &P, bool learning, SourceContextManager &M, bool naive): P(P), learning(learning), M(M),
    ...
    virtual std::vector<unsigned long> preprocess(const RepairCandidate &candidate) {
        outlog_printf(2, "Preprocess the following candidate with BasicTester:\n%s\n",
        ...
        for (std::set<ExprFillInfo>::iterator it = infos.begin(); it != infos.end();
            ...
            {
                outlog_printf(2, "BasicTester, a patch instance with id %lu:\n",
                        codes.size());
            ...
            if (!naive) {
                ...
                if (D.hasDuplicateCodeToPatch()) {
                    ...
                    {
                        outlog_printf(2, "BasicTester, a patch instance with id %lu (duplicate):\n",
                                codes.size());
...
public:
    ...
    virtual std::vector<unsigned long> preprocess(const RepairCandidate &candidate) {
        outlog_printf(2, "Preprocess the following candidate with StringConstTester:\n%s\n",
                candidate.toString(M).c_str());
        // We are going to create a set of binding ExprFillInfos
        ...
        for (std::set<ExprFillInfo>::iterator it = the_infos.begin(); it != the_infos.end();
            ...
            CodeSegTy a_patch = R.getPatches();
            {
                outlog_printf(2, "StringConstTester, a patch instance with id %lu:\n",
                        codes.size());
                out_codes(a_code, a_patch);
            }
            res.push_back((unsigned long)codes.size());
            ...
            if (!naive) {
                ...
                if (D.hasDuplicateCodeToPatch()) {
                    ...
                    CodeSegTy a_new_patch = D.getNewPatches();
                    {
                        outlog_printf(2, "StringConstTester, a patch instance with id %lu (duplicate):\n",
                                codes.size());
                        out_codes(a_new_code, a_new_patch);
                    }
                    res.push_back(codes.size());
...
public:
    ConditionSynthesisTester(BenchProgram &P, bool learning, SourceContextManager &M, bool full_synthesis):
        ...
        global_cnt(0), infos(), full_synthesis(full_synthesis) { post_cnt = 0; }
    ...
    virtual std::vector<unsigned long> preprocess(const RepairCandidate &candidate) {
        outlog_printf(2, "Preprocess the following candidate with CondTester:\n%s\n",
                candidate.toString(M).c_str());
        // We identify the part we can use condition synthesizer, for the rest
        // we just brute search.
        ...
        for (std::set<ExprFillInfo>::iterator it = the_infos.begin(); it != the_infos.end();
            ...
            {
                outlog_printf(2, "CondTester, a patch instance with id %lu:\n",
                        codes.size());
                out_codes(a_code, a_patch);
            }
            candidates.push_back(candidate);

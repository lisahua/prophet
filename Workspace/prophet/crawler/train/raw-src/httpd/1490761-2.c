# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 313 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
# 17 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
...
# 18 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
apr_status_t apr_password_get(const char *prompt, char *pwbuf,
                                           apr_size_t *bufsize);
# 19 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
# 20 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
apr_status_t apr_temp_dir_get(const char **temp_dir,
                                           apr_pool_t *p);
# 22 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
# 26 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
# 27 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
apr_status_t apr_pollcb_poll(apr_pollcb_t *pollcb,
                                          ...
                                          void *baton);
# 29 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
# 33 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
...
# 36 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c" 2
# 46 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
...
static int get_now(rotate_config_t *config)
{
    ...
    return (int)((tNow) / 1000000L) + utc_offset;
}
...
# 216 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
static void checkRotate(rotate_config_t *config, rotate_status_t *status)
{
    if (status->current.fd == ((void*)0)) {
        status->rotateReason = 1;
    }
    else if (config->sRotation) {
        apr_finfo_t finfo;
        apr_off_t current_size = -1;

        if (apr_file_info_get(&finfo, 0x00000100, status->current.fd) == 0) {
            current_size = finfo.size;
        }

        if (current_size > config->sRotation) {
            status->rotateReason = 3;
        }
        else if (config->tRotation) {
            if (get_now(config) >= status->tLogEnd) {
                status->rotateReason = 2;
            }
        }
    }
    else if (config->tRotation) {
    ...
    if (status->rotateReason != 0 && config->verbose) {
                        ...
                        rotate_config_t *config, rotate_status_t *status)
{
    ...
    while (apr_proc_wait_all_procs(&proc, ((void*)0), ((void*)0), APR_NOWAIT, pool) == ((20000 + 50000) + 5))
                  ;

    if ((rv = apr_procattr_create(&pattr, pool)) != 0) {
        fprintf(stderr,
    ...
    rv = apr_proc_create(&proc, argv[0], argv, ((void*)0), pattr, pool);
    if (rv != 0) {
...
static void truncate_and_write_error(rotate_status_t *status)
{
    ...
    if (apr_file_trunc(status->current.fd, 0) != 0) {
        fprintf(stderr, "Error truncating the file %s\n", status->current.name);
        exit(2);
    }
    if (apr_file_write_full(status->current.fd, status->errbuf, buflen, ((void*)0)) != 0) {
        fprintf(stderr, "Error writing error (%s) to the file %s\n",
                status->errbuf, status->current.name);
        exit(2);
    }
}
# 360 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
static void doRotate(rotate_config_t *config, rotate_status_t *status)
{
    ...
    if (config->use_strftime) {
        apr_time_t tNow = ((apr_time_t)(tLogStart) * 1000000L);
        apr_time_exp_t e;
        apr_size_t rs;
    ...
    apr_pool_create_ex(&newlog.pool, status->pool, ((void*)0), ((void*)0));
    if (config->verbose) {
        fprintf(stderr, "Opening file %s\n", newlog.name);
    }
    rv = apr_file_open(&newlog.fd, newlog.name, 0x00002 | 0x00004 | 0x00008
                       | (config->truncate || (config->num_files > 0 && status->current.fd) ? 0x00010 : 0),
                       0x0FFF, newlog.pool);
    if (rv == 0) {
...
# 473 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
static const char *get_time_or_size(rotate_config_t *config,
                                    const char *arg, int last) {
    char *ptr = ((void*)0);

    unsigned int mult = 1;
    if ((ptr = (__extension__ (__builtin_constant_p ('B') && !__builtin_constant_p (arg) && ('B') == '\0' ? (char *) __rawmemchr (arg, 'B') : __builtin_strchr (arg, 'B')))) != ((void*)0)) {
        mult = 1;
    }
    else if ((ptr = (__extension__ (__builtin_constant_p ('K') && !__builtin_constant_p (arg) && ('K') == '\0' ? (char *) __rawmemchr (arg, 'K') : __builtin_strchr (arg, 'K')))) != ((void*)0)) {
        mult = 1024;
    }
    else if ((ptr = (__extension__ (__builtin_constant_p ('M') && !__builtin_constant_p (arg) && ('M') == '\0' ? (char *) __rawmemchr (arg, 'M') : __builtin_strchr (arg, 'M')))) != ((void*)0)) {
        mult = 1024 * 1024;
    }
    else if ((ptr = (__extension__ (__builtin_constant_p ('G') && !__builtin_constant_p (arg) && ('G') == '\0' ? (char *) __rawmemchr (arg, 'G') : __builtin_strchr (arg, 'G')))) != ((void*)0)) {
        mult = 1024 * 1024 * 1024;
    }
    if (ptr) {
...
int main (int argc, const char * const argv[])
{
    ...
    apr_pool_create_ex(&status.pool, ((void*)0), ((void*)0), ((void*)0));
    apr_getopt_init(&opt, status.pool, argc, argv);

    while ((rv = apr_getopt(opt, "lL:p:ftvecn:", &c, &opt_arg)) == 0) {
    ...
    for(; opt->ind < argc; opt->ind++) {
        if ((err = get_time_or_size(&config, argv[opt->ind],
                                    opt->ind < argc - 1 ? 0 : 1)) != ((void*)0)) {
            usage(argv[0], err);
        }
    }

    config.use_strftime = ((__extension__ (__builtin_constant_p ('%') && !__builtin_constant_p (config.szLogRoot) && ('%') == '\0' ? (char *) __rawmemchr (config.szLogRoot, '%') : __builtin_strchr (config.szLogRoot, '%'))) != ((void*)0));
    ...
    if (apr_file_open_stdin(&f_stdin, status.pool) != 0) {
        fprintf(stderr, "Unable to open stdin\n");
        exit(1);
    }

    if (apr_file_open_stdout(&f_stdout, status.pool) != 0) {
    ...
    for (;;) {
        ...
        if (config.create_empty && config.tRotation) {
            ...
            if (polltimeout <= 0) {
                pollret = ((20000 + 50000) + 7);
            }
            else {
                pollret = apr_poll(&pollfd, 1, &pollret, ((apr_time_t)(polltimeout) * 1000000L));
            }
        }
        if (pollret == 0) {
...
# 692 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/support/rotatelogs.c"
        checkRotate(&config, &status);
        if (status.rotateReason != 0) {
            doRotate(&config, &status);
        }
        ...
        if (nWrite != nRead) {
            ...
            cur_offset = 0;
            if (apr_file_seek(status.current.fd, 1, &cur_offset) != 0) {
                cur_offset = -1;
            }
            status.nMessCount++;
            apr_snprintf(status.errbuf, sizeof status.errbuf,
                         "Error %d writing to log file at offset %" "ld" ". "

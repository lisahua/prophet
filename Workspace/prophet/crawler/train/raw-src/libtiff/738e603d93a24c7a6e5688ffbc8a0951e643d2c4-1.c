# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_flush.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_flush.c" 2
# 30 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_flush.c"
# 1 "./tiffiop.h" 1
# 33 "./tiffiop.h"
# 1 "./tif_config.h" 1
# 34 "./tiffiop.h" 2
...
# 31 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_flush.c" 2
...
TIFFFlush(TIFF* tif)
{
        ...
        && tif->tif_mode == 02 )
    {
        ...
        if( TIFFIsTiled(tif) )
        {
            if( TIFFGetField( tif, 324, &offsets )
                && TIFFGetField( tif, 325, &sizes )
                && _TIFFRewriteField( tif, 324, TIFF_LONG8,
                                      tif->tif_dir.td_nstrips, offsets )
                && _TIFFRewriteField( tif, 325, TIFF_LONG8,
        ...
        {
            if( TIFFGetField( tif, 273, &offsets )
                && TIFFGetField( tif, 279, &sizes )
                && _TIFFRewriteField( tif, 273, TIFF_LONG8,
                                      tif->tif_dir.td_nstrips, offsets )
                && _TIFFRewriteField( tif, 279, TIFF_LONG8,
                                      tif->tif_dir.td_nstrips, sizes ) )
            {
                tif->tif_flags &= ~0x200000;
                tif->tif_flags &= ~0x00040;
    ...
    if ((tif->tif_flags & (0x00008|0x200000))
        && !TIFFWriteDirectory(tif))
        return (0);

    return (1);
}
# 98 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_flush.c"
int
TIFFFlushData(TIFF* tif)
{
 if ((tif->tif_flags & 0x00040) == 0)
  return (1);

# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c" 2
# 32 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c"
# 1 "./tiffiop.h" 1
# 33 "./tiffiop.h"
# 1 "./tif_config.h" 1
# 34 "./tiffiop.h" 2
...
# 33 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c" 2
...
# 36 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c" 2
...
TIFFPrintDirectory(TIFF* tif, FILE* fd, long flags)
{
 ...
 if ((((tif)->tif_dir.td_fieldsset[(5)/32]) & (((unsigned long)1L)<<((5)&0x1f)))) {
  ...
  sep = " ";
  if (td->td_subfiletype & 0x1) {
   fprintf(fd, "%sreduced-resolution image", sep);
   sep = "/";
  }
  if (td->td_subfiletype & 0x2) {
   fprintf(fd, "%smulti-page document", sep);
   sep = "/";
  }
  if (td->td_subfiletype & 0x4)
 ...
 if ((((tif)->tif_dir.td_fieldsset[(31)/32]) & (((unsigned long)1L)<<((31)&0x1f))) && td->td_extrasamples) {
 ...
 if ((((tif)->tif_dir.td_fieldsset[(46)/32]) & (((unsigned long)1L)<<((46)&0x1f)))) {
  ...
  sep = "";
  for (cp = td->td_inknames; i > 0; cp = (__extension__ (__builtin_constant_p ('\0') && !__builtin_constant_p (cp) && ('\0') == '\0' ? (char *) __rawmemchr (cp, '\0') : __builtin_strchr (cp, '\0')))+1, i--) {
   fputs(sep, fd);
 ...
 if ((((tif)->tif_dir.td_fieldsset[(15)/32]) & (((unsigned long)1L)<<((15)&0x1f)))) {
  ...
  if (td->td_orientation < (sizeof (orientNames) / sizeof (orientNames[0])))
 ...
 if ((((tif)->tif_dir.td_fieldsset[(49)/32]) & (((unsigned long)1L)<<((49)&0x1f))) && (td->td_subifd)) {
...
# 591 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/libtiff/tif_print.c"
     char *tmp;
     raw_data = _TIFFmalloc(
         _TIFFDataSize(fip->field_type)
         * value_count);
...
_TIFFprintAscii(FILE* fd, const char* cp)
{
 for (; *cp != '\0'; cp++) {
  ...
  if (((*__ctype_b_loc ())[(int) (((int)*cp))] & (unsigned short int) _ISprint)) {
   fputc(*cp, fd);
   continue;
  }
  for (tp = "\tt\bb\rr\nn\vv"; *tp; tp++)

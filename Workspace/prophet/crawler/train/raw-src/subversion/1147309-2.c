             ...
             apr_pool_t *pool)
{
  ...
  if ((! srcs_are_urls) && (! dst_is_url))
  ...
  else if ((! srcs_are_urls) && (dst_is_url))
    {
      ...
      if (! opt_state->quiet)
        do { svn_error_t *svn_err__temp = (svn_cl__notifier_mark_wc_to_repos_copy(ctx->notify_baton2)); if (svn_err__temp) return (svn_err__temp); } while (0);
    }
  else if ((srcs_are_urls) && (! dst_is_url))
    {

     if (! opt_state->quiet)
       do { svn_error_t *svn_err__temp = (svn_cl__notifier_mark_checkout(ctx->notify_baton2)); if (svn_err__temp) return (svn_err__temp); } while (0);
    }
  else
    {
  ...
  err = svn_client_copy6(sources, dst_path, (!0),

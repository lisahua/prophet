...
svn_wc_merge(const char *left,
             ...
             const char *diff3_cmd,
             apr_pool_t *pool);
# 6865 "./subversion/include/svn_wc.h"
svn_error_t *
...
svn_wc_transmit_text_deltas(const char *path,
                            ...
                            const char **tempfile,
                            apr_pool_t *pool);
# 7584 "./subversion/include/svn_wc.h"
svn_error_t *
...
svn_wc_add_lock2(svn_wc_context_t *wc_ctx,
                 ...
                 const svn_lock_t *lock,
                 apr_pool_t *scratch_pool);
# 7697 "./subversion/include/svn_wc.h"
__attribute__((deprecated))
svn_error_t *
svn_wc_add_lock(const char *path,
                ...
                svn_wc_adm_access_t *adm_access,
                apr_pool_t *pool);
# 7711 "./subversion/include/svn_wc.h"
svn_error_t *
...
svn_wc_set_changelist(const char *path,
                      ...
                      void *notify_baton,
                      apr_pool_t *pool);
# 7888 "./subversion/include/svn_wc.h"
           ...
           apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Path: %s\n", 5), svn_cl__local_style_skip_ancestor( path_prefix, target, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (info->URL && info->repos_root_URL)
    do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Relative URL: ^/%s\n", 5), svn_path_uri_encode( svn_uri_skip_ancestor(info->repos_root_URL, info->URL, pool), pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
      ...
      if (info->wc_info->moved_from_abspath)
        {
          ...
          relpath = svn_dirent_skip_ancestor(info->wc_info->wcroot_abspath,
                                             info->wc_info->moved_from_abspath);
          if (relpath && relpath[0] != '\0')
            do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Moved From: %s\n", 5), relpath)); if (svn_err__temp) return (svn_err__temp); } while (0);
          else
            do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Moved From: %s\n", 5), info->wc_info->moved_from_abspath)); if (svn_err__temp) return (svn_err__temp); } while (0);
      ...
      if (info->wc_info->moved_to_abspath)
        {
          ...
          relpath = svn_dirent_skip_ancestor(info->wc_info->wcroot_abspath,
                                             info->wc_info->moved_to_abspath);
          if (relpath && relpath[0] != '\0')
            do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Moved To: %s\n", 5), relpath)); if (svn_err__temp) return (svn_err__temp); } while (0);
          else
            do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Moved To: %s\n", 5), info->wc_info->moved_to_abspath)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (info->wc_info)
    {
      if (info->wc_info->recorded_time)
        do { svn_error_t *svn_err__temp = (svn_cl__info_print_time(info->wc_info->recorded_time, dcgettext ("subversion", "Text Last Updated", 5), pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
              ...
              switch (conflict->kind)
                {
                  case svn_wc_conflict_kind_text:
                    if (conflict->base_abspath)
                      do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Previous Base File: %s\n", 5), svn_cl__local_style_skip_ancestor( path_prefix, conflict->base_abspath, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
                    ...
                    if (conflict->my_abspath)
                      do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Previous Working File: %s\n", 5), svn_cl__local_style_skip_ancestor( path_prefix, conflict->my_abspath, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
                    ...
                    if (conflict->their_abspath)
                      do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Current Base File: %s\n", 5), svn_cl__local_style_skip_ancestor( path_prefix, conflict->their_abspath, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);

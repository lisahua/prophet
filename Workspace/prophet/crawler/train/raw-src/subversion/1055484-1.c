               ...
               apr_pool_t *pool)
{
  ...
  if (info->conflict_old || info->conflict_wrk
      || info->conflict_new || info->prejfile)
    {
      ...
      svn_xml_make_open_tag(&sb, pool, svn_xml_normal, "conflict", ((void*)0));
  ...
  if (info->tree_conflict)
    do { svn_error_t *svn_err__temp = (svn_cl__append_tree_conflict_info_xml(sb, info->tree_conflict, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
           ...
           apr_pool_t *pool)
{
  ...
  if (info->has_wc_info)
    {
      if (info->text_time)
        do { svn_error_t *svn_err__temp = (svn_cl__info_print_time(info->text_time, dcgettext ("subversion", "Text Last Updated", 5), pool)); if (svn_err__temp) return (svn_err__temp); } while (0);


      if (info->checksum)
        do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Checksum: %s\n", 5), info->checksum)); if (svn_err__temp) return (svn_err__temp); } while (0);


      if (info->conflict_old)
        do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Previous Base File: %s\n", 5), svn_dirent_local_style(info->conflict_old, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);




      if (info->conflict_wrk)
        do { svn_error_t *svn_err__temp = (svn_cmdline_printf (pool, dcgettext ("subversion", "Conflict Previous Working File: %s\n", 5), svn_dirent_local_style(info->conflict_wrk, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);



      if (info->conflict_new)
        do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Current Base File: %s\n", 5), svn_dirent_local_style(info->conflict_new, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);




      if (info->prejfile)
        do { svn_error_t *svn_err__temp = (svn_cmdline_printf(pool, dcgettext ("subversion", "Conflict Properties File: %s\n", 5), svn_dirent_local_style(info->prejfile, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (info->tree_conflict)
    {
      ...
      do { svn_error_t *svn_err__temp = (svn_cl__get_human_readable_tree_conflict_description( &desc, info->tree_conflict, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
             ...
             apr_pool_t *pool)
{
      ...
      if (err)
        {
          ...
          if (err->apr_err == SVN_ERR_UNVERSIONED_RESOURCE
            ...
            {
              do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf (stderr, subpool, dcgettext ("subversion", "%s:  (Not a versioned resource)\n\n", 5), svn_path_is_url(truepath) ? truepath : svn_dirent_local_style(truepath, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
            ...
            {
              do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf (stderr, subpool, dcgettext ("subversion", "%s:  (Not a valid URL)\n\n", 5), svn_path_is_url(truepath) ? truepath : svn_dirent_local_style(truepath, pool))); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (saw_a_problem)
    return svn_error_create(SVN_ERR_BASE, ((void*)0), ((void*)0));
  else
    return 0;
}

  ...
  if (remove_unversioned_items || remove_ignored_items || include_externals)
    {
      ...
      if (remove_ignored_items)
        do { svn_error_t *svn_err__temp = (svn_wc_get_ignores2(&ignores, ctx->wc_ctx, local_abspath, ctx->config, scratch_pool, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

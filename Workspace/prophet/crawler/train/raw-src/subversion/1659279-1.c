            ...
            if (!printed_tc)
              {
                ...
                do { svn_error_t *svn_err__temp = (svn_cl__get_human_readable_action_description(&desc, svn_wc_conflict_reason_edited, conflict->operation, conflict->node_kind, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

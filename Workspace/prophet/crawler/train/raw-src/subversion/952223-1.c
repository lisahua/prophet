                       ...
                       apr_pool_t *scratch_pool)
{
  ...
  *status = memset(apr_palloc(result_pool, sizeof(**status)), 0, sizeof(**status));
  ...
  if (old_status->versioned)
    do { svn_error_t *svn_err__temp = (svn_wc__get_entry(&entry, wc_ctx->db, local_abspath, (!0), svn_node_unknown, 0, result_pool, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

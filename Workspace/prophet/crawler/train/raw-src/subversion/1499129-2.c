      ...
      if (err && err->apr_err == SVN_ERR_WC_LOCKED)
          ...
          if (err2)
          ...
          else if (opt_state->remove_unversioned || opt_state->remove_ignored)
            {
              err = svn_error_create(SVN_ERR_WC_LOCKED, err,
                                     dcgettext ("subversion", "Working copy locked; if no other " "Subversion client is currently " "using the working copy, try running " "'svn cleanup' without the " "--remove-unversioned and " "--remove-ignored options first.", 5));
               ...
               (opt_state->remove_unversioned || opt_state->remove_ignored))
        {
          err = svn_error_create(SVN_ERR_CL_ARG_PARSING_ERROR, err,
                                 dcgettext ("subversion", "Cannot remove unversioned or ignored " "items from something that is not a " "working copy", 5));

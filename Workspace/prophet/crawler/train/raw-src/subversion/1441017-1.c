  ...
  if (opt_state->use_merge_history)
    {
      ...
      if (merged_revision < revision)
        {
          do { svn_error_t *svn_err__temp = (svn_stream_puts(out, "G ")); if (svn_err__temp) return (svn_err__temp); } while (0);
          use_merged = (!0);
        }
      else
        do { svn_error_t *svn_err__temp = (svn_stream_puts(out, "  ")); if (svn_err__temp) return (svn_err__temp); } while (0);
    }

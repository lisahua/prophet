               ...
               apr_pool_t *scratch_pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_opt_parse_path(&peg_revision, &switch_url, (((const char * *)(targets)->elts)[0]), scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (err)
    {
      if (err->apr_err == SVN_ERR_CLIENT_UNRELATED_RESOURCES)
        return svn_error_createf(SVN_ERR_CLIENT_UNRELATED_RESOURCES, err,
                                 dcgettext ("subversion", "Path '%s' does not share common version " "control ancestry with the requested switch " "location.  Use --ignore-ancestry to " "disable this check.", 5),

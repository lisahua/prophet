...
# 37 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c" 2
...
svn_wc__diff7(const char **root_relpath,
              ...
              apr_pool_t *result_pool,
              apr_pool_t *scratch_pool);
# 41 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c" 2
...
# 68 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c"
static svn_error_t *
                     ...
                     svn_error_t *ra_err, apr_pool_t *pool)
{
  ...
  const char *local_abspath = lb->urls_to_paths
                                  ? apr_hash_get(lb->urls_to_paths, rel_url, (-1))
                                  : ((void*)0);
...
# 148 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c"
static svn_error_t *
...
# 233 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c"
static svn_error_t *
                      ...
                      apr_pool_t *scratch_pool)
{
  ...
  *lock_paths = ((void*)0);
  ...
  do { if (!(targets->nelts)) do { svn_error_t *svn_err__temp = (svn_error__malfunction((!0), "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_client/locking_commands.c", 254, "targets->nelts")); if (svn_err__temp) return (svn_err__temp); } while (0); } while (0);
  ...
  if (url_mode)
    {
      ...
      *invalid_revnum = ((svn_revnum_t) -1);
      ...
      for (i = 0; i < rel_targets->nelts; i++)
        {
          apr_hash_set(rel_targets_ret, (((const char * *)(rel_targets)->elts)[i]), (-1), do_lock ? (const void *)invalid_revnum : (const void *)"");
      ...
      *lock_paths = apr_array_make(result_pool, 1, sizeof(const char *));
      ...
      for (i = 0; i < targets->nelts; i++)
        {
          ...
          do { svn_error_t *svn_err__temp = (svn_dirent_get_absolute(&target_abspath, (((const char* *)(targets)->elts)[i]), result_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);



          do { svn_error_t *svn_err__temp = (svn_wc__get_wcroot(&wcroot_abspath, wc_ctx, target_abspath, iterpool, iterpool)); if (svn_err__temp) return (svn_err__temp); } while (0);


          wc_targets = apr_hash_get(wcroot_target, wcroot_abspath, (-1));

          if (!wc_targets)
            {
              wc_targets = apr_array_make(scratch_pool, 1, sizeof(const char*));
              apr_hash_set(wcroot_target, apr_pstrdup(scratch_pool, wcroot_abspath), (-1), wc_targets);
           ...
           hi = apr_hash_next(hi))
        {
          ...
          if (paths->nelts == 1)
            lock_abspath = svn_dirent_dirname(
                                (((const char * *)(paths)->elts)[0]),
                                result_pool);
          else
            do { svn_error_t *svn_err__temp = (svn_dirent_condense_targets(&lock_abspath, ((void*)0), paths, 0, result_pool, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);



          do { svn_error_t *svn_err__temp = (svn_wc__acquire_write_lock(&lock_abspath, wc_ctx, lock_abspath, 0, result_pool, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);



          (*((const char * *)apr_array_push(*lock_paths))) = lock_abspath;
        }
          ...
          for (i = 0; i < wc_targets->nelts; i++)
            {
              ...
              local_abspath = (((const char * *)(wc_targets)->elts)[i]);
              wli = memset(apr_palloc(scratch_pool, sizeof(*wli)), 0, sizeof(*wli));

              do { svn_error_t *svn_err__temp = (svn_wc__node_get_base(&kind, &wli->revision, &repos_relpath, &repos_root_url, ((void*)0), &wli->lock_token, wc_ctx, local_abspath, 0 , result_pool, iterpool)); if (svn_err__temp) return (svn_err__temp); } while (0);
              ...
              if (kind != svn_node_file)
                return svn_error_createf(SVN_ERR_WC_NOT_FILE, ((void*)0),
                                         dcgettext ("subversion", "The node '%s' is not a file", 5),
              ...
              wli->url = svn_path_url_add_component2(repos_root_url,
                                                     repos_relpath,
                                                     scratch_pool);
              apr_hash_set(wc_info, local_abspath, (-1), wli);

              (*((const char * *)apr_array_push(target_urls))) = wli->url;
            }
        }
      ...
      for (hi = apr_hash_first(scratch_pool, wc_info);
           ...
           hi = apr_hash_next(hi))
        {
          ...
          apr_hash_set(rel_fs_paths, rel_url, (-1), apr_pstrdup(result_pool, local_abspath));
          ...
          if (do_lock)
            {
              ...
              *revnum = wli->revision;
              ...
              apr_hash_set(rel_targets_ret, rel_url, (-1), revnum);
            }
          else
            {
              const char *lock_token;
              ...
              apr_hash_set(rel_targets_ret, rel_url, (-1), lock_token ? lock_token : "");
  ...
  *common_parent_url = common_url;
  if (*lock_paths && (*lock_paths)->nelts == 1)
    *base_dir_abspath = (((const char* *)(*lock_paths)->elts)[0]);
  else
  ...
  for (hi = apr_hash_first(pool, path_tokens); hi; hi = apr_hash_next(hi))
    {
      ...
      apr_hash_set(path_tokens, path, (-1), apr_pstrdup(pool, lock->token));
    }
      ...
      for (i = 0; i < lock_abspaths->nelts; i++)
        {
          err = svn_error_compose_create(
                  ...
                  svn_wc__release_write_lock(ctx->wc_ctx,
                                             (((const char * *)(lock_abspaths)->elts)[i]),

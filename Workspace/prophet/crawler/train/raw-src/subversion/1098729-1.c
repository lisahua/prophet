                       ...
                       apr_pool_t *scratch_pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_wc__node_is_file_external(&file_external, ctx->wc_ctx, local_abspath, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);


  if (file_external)
    return svn_error_createf(SVN_ERR_WC_CANNOT_DELETE_FILE_EXTERNAL, ((void*)0),
                             dcgettext ("subversion", "Cannot remove the file external at '%s'; " "please propedit or propdel the svn:externals " "description that created it", 5),
            ...
            apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_uri_condense_targets(&common, &targets, paths, (!0), pool, subpool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (((ctx)->log_msg_func3 || (ctx)->log_msg_func2 || (ctx)->log_msg_func))
    {
      ...
      for (i = 0; i < targets->nelts; i++)
        {
          const char *relpath = (((const char * *)(targets)->elts)[i]);
  ...
  for (i = 0; i < targets->nelts; i++)
    {
      ...
      if (! ra_session)
        {
          do { svn_error_t *svn_err__temp = (svn_client__open_ra_session_internal(&ra_session, ((void*)0), item_url, ((void*)0), ((void*)0), 0, (!0), ctx, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);


        }
      else
        {
          do { svn_error_t *svn_err__temp = (svn_ra_reparent(ra_session, item_url, subpool)); if (svn_err__temp) return (svn_err__temp); } while (0);
        }

      do { svn_error_t *svn_err__temp = (svn_ra_check_path(ra_session, "", ((svn_revnum_t) -1), &kind, subpool)); if (svn_err__temp) return (svn_err__temp); } while (0);

      if (kind == svn_node_none)
        return svn_error_createf(SVN_ERR_FS_NOT_FOUND, ((void*)0),
                                 dcgettext ("subversion", "URL '%s' does not exist", 5), item_url);
    }
  apr_pool_destroy(subpool);
                   ...
                   apr_pool_t *pool)
{
  ...
  if (is_url)
    {
      do { svn_error_t *svn_err__temp = (delete_urls(paths, revprop_table, commit_callback, commit_baton, ctx, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

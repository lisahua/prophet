                     ...
                     apr_pool_t *scratch_pool)
{
  ...
  if (is_copy)
    ...
    tmpinfo->rev = rev;
# 125 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_wc/info.c"
  if (! ((tmpinfo->rev) >= 0))
    tmpinfo->rev = 0;
                         ...
                         apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_wc__db_op_read_tree_conflict(&tree_conflict, fe_baton->wc_ctx->db, local_abspath, pool, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  do { if (!(info != ((void*)0) && info->wc_info != ((void*)0))) do { svn_error_t *svn_err__temp = (svn_error__malfunction((!0), "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_wc/info.c", 289, "info != NULL && info->wc_info != NULL")); if (svn_err__temp) return (svn_err__temp); } while (0); } while (0);

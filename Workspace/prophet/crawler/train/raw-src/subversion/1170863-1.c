...
# 92 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_delta/compat.c"
                 ...
                 apr_pool_t *scratch_pool)
{
  ...
  *revnum = revision;
  do { svn_error_t *svn_err__temp = (add_action(pb->eb, path, delete, revnum)); if (svn_err__temp) return (svn_err__temp); } while (0);
                  ...
                  void **child_baton)
{
  ...
  *kind = svn_node_dir;
  do { svn_error_t *svn_err__temp = (add_action(pb->eb, path, add, kind)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (value)
    {
      ...
      do { svn_error_t *svn_err__temp = (add_action(db->eb, db->path, set_prop, p_args)); if (svn_err__temp) return (svn_err__temp); } while (0);
    }
  else
    do { svn_error_t *svn_err__temp = (add_action(db->eb, db->path, remove_prop, apr_pstrdup(db->eb->edit_pool, name))); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  if (value)
    {
      ...
      do { svn_error_t *svn_err__temp = (add_action(fb->eb, fb->path, set_prop, p_args)); if (svn_err__temp) return (svn_err__temp); } while (0);
    }
  else
    do { svn_error_t *svn_err__temp = (add_action(fb->eb, fb->path, remove_prop, apr_pstrdup(fb->eb->edit_pool, name))); if (svn_err__temp) return (svn_err__temp); } while (0);
...
{
  struct ev2_edit_baton *eb = edit_baton;
  apr_array_header_t *removed_props = ((void*)0);
  ...
  for (i = 0; i < actions->nelts; i++)
    {
      struct path_action *action = (((struct path_action * *)(actions)->elts)[i]);
          ...
          case remove_prop:
            {
              ...
              (*((const char * *)apr_array_push(removed_props))) = name;
              break;
            }
  ...
  if (props || removed_props)
    {
      ...
      do { svn_error_t *svn_err__temp = (eb->fetch_props_func(&existing_props, eb->fetch_props_baton, path, scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
      ...
      if (removed_props)
        {
          for (i = 0; i < removed_props->nelts; i++)
            {
              const char *name = (((const char * *)(removed_props)->elts)[i]);
      ...
      do { svn_error_t *svn_err__temp = (svn_editor_set_props(eb->editor, path, ((svn_revnum_t) -1), props, (!0))); if (svn_err__temp) return (svn_err__temp); } while (0);
...
# 766 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_delta/compat.c"
  return 0;
}

                                ...
                                apr_pool_t *result_pool)
{
  ...
  version = memset(apr_palloc(result_pool, sizeof(*version)), 0, sizeof(*version));
    ...
    do { if (!(svn_uri_is_canonical(repos_url, result_pool) && svn_relpath_is_canonical(repos_relpath) && ((revision) >= 0))) { svn_error__malfunction(0, "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_wc/util.c", 302, "svn_uri_is_canonical(repos_url, result_pool) && svn_relpath_is_canonical(repos_relpath) && SVN_IS_VALID_REVNUM(revision)"); abort(); } } while (0);
                        ...
                        apr_pool_t *scratch_pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_wc__db_read_kind(kind, sfb->db, local_abspath, 0 , 0 , scratch_pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

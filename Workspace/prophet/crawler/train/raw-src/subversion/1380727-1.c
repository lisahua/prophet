...
main(int argc, const char **argv)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_utf_cstring_to_utf8(&authz_file, argv[1], pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svn: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
  authz_file = svn_dirent_internal_style(authz_file, pool);


  err = svn_repos_authz_read(&authz, authz_file, (!0), pool);

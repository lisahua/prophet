                           ...
                           apr_pool_t *pool)
{
  ...
  do { const void *keystr = ("svn:realmstring"); apr_hash_set(hash, keystr, strlen(keystr), svn_string_create(realmstring, pool)); } while (0);


  do { svn_error_t *svn_err__temp = (svn_io_file_open(&authfile, auth_path, (0x00002 | 0x00004 | 0x00010 | 0x00080), 0x0FFF, pool)); if (svn_err__temp) return svn_error_quick_wrap(svn_err__temp, dcgettext ("subversion", "Unable to open auth file for writing", 5)); } while (0);





  stream = svn_stream_from_aprfile2(authfile, 0, pool);
  ...
  do { const void *keystr = ("svn:realmstring"); apr_hash_set(hash, keystr, strlen(keystr), ((void*)0)); } while (0);

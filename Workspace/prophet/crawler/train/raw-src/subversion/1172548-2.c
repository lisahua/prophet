             ...
             apr_pool_t *pool)
{
  ...
  if (! opt_state->old_target && ! opt_state->new_target
  ...
  else if (opt_state->old_target)
    {
      ...
      do { svn_error_t *svn_err__temp = (svn_cl__args_to_target_array_print_reserved(&tmp2, os, tmp, ctx, 0, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  else
    {
      ...
      do { svn_error_t *svn_err__temp = (svn_cl__assert_homogeneous_target_type(targets)); if (svn_err__temp) return (svn_err__temp); } while (0);

      working_copy_present = ! svn_path_is_url((((const char * *)(targets)->elts)[0]));
...
# 353 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/svn/diff-cmd.c"
          else
            do { svn_error_t *svn_err__temp = (svn_client_diff6( options, target1, &(opt_state->start_revision), target2, &(opt_state->end_revision), ((void*)0), opt_state->depth, ! opt_state->notice_ancestry, opt_state->no_diff_deleted, opt_state->show_copies_as_adds, opt_state->force, opt_state->use_git_diff_format, svn_cmdline_output_encoding(pool), outstream, errstream, opt_state->changelists, ctx, iterpool)); if (svn_err__temp) return (svn_err__temp); } while (0);
# 372 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/svn/diff-cmd.c"
        }
      else
        {
          const char *truepath;
...
# 398 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/svn/diff-cmd.c"
          else
            do { svn_error_t *svn_err__temp = (svn_client_diff_peg6( options, truepath, &peg_revision, &opt_state->start_revision, &opt_state->end_revision, ((void*)0), opt_state->depth, ! opt_state->notice_ancestry, opt_state->no_diff_deleted, opt_state->show_copies_as_adds, opt_state->force, opt_state->use_git_diff_format, svn_cmdline_output_encoding(pool), outstream, errstream, opt_state->changelists, ctx, iterpool)); if (svn_err__temp) return (svn_err__temp); } while (0);
# 417 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/svn/diff-cmd.c"
        }
    }

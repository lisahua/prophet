  ...
  for (i = 0; i < targets->nelts; i++)
    {
      ...
      if (i < result_revs->nelts)
        {
          svn_revnum_t rev = (((const char * *)(result_revs)->elts)[i]);

          if (((rev) >= 0))
            do { svn_error_t *svn_err__temp = (svn_cmdline_printf(scratch_pool, dcgettext ("subversion", "  Updated '%s' to r%ld.\n", 5), path, rev)); if (svn_err__temp) return (svn_err__temp); } while (0);
               ...
               apr_pool_t *scratch_pool)
{
  ...
  if (nwb.had_externals_error)
    return (svn_error__locate("/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svn/update-cmd.c",146), (svn_error_create))(SVN_ERR_CL_ERROR_PROCESSING_EXTERNALS, ((void*)0),

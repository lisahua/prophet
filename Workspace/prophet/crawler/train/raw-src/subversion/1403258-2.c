                                ...
                                apr_pool_t *result_pool)
{
  ...
  version = memset(apr_palloc(result_pool, sizeof(*version)), 0, sizeof(*version));
    ...
    do { if (!(svn_uri_is_canonical(repos_url, result_pool) && svn_relpath_is_canonical(repos_relpath) && ((revision) >= 0))) { svn_error__malfunction(0, "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/subversion/libsvn_wc/util.c", 302, "svn_uri_is_canonical(repos_url, result_pool) && svn_relpath_is_canonical(repos_relpath) && SVN_IS_VALID_REVNUM(revision)"); abort(); } } while (0);

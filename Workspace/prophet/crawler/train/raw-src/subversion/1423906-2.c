             ...
             apr_pool_t *pool)
{
  ...
  {
    const char *tgt = (((const char * *)(targets)->elts)[targets->nelts - 1]);
    svn_opt_revision_t peg;

    do { svn_error_t *svn_err__temp = (svn_opt_parse_path(&peg, &dst_path, tgt, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  err = svn_client_copy6(sources, dst_path, (!0),
                         ...
                         (opt_state->quiet ? ((void*)0) : svn_cl__print_commit_info),
                         ((void*)0),
                         ctx, pool);

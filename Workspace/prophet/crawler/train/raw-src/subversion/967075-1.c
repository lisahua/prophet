...
# 34 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svnrdump/dump_editor.h"
struct dir_baton
{
...
static const apr_getopt_option_t svnrdump__options[] =
  {
    ...
    {"config-dir", opt_config_dir, 1, "read user configuration files from" " directory ARG" },

    {"username", opt_auth_username, 1, "specify a username ARG"},
    {"password", opt_auth_password, 1, "specify a password ARG"},
    {"non-interactive", opt_non_interactive, 0, "do no interactive" " prompting"},

    {"no-auth-cache", opt_auth_nocache, 0, "do not cache authentication" " tokens"},
  ...
  if (start_revision == 0)
    {
      ...
      do { svn_error_t *svn_err__temp = (svn_ra_rev_proplist(session, start_revision, &encoded_prophash, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
# 234 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svnrdump/svnrdump.c"
      apr_hash_set(encoded_prophash, "svn:" "sync-" "from-url",
                   (-1), svn_string_create(url, pool));
      apr_hash_set(encoded_prophash, "svn:" "sync-" "from-uuid",
                   (-1), svn_string_create(uuid, pool));

      propstream = svn_stream_from_stringbuf(propstring, pool);
      do { svn_error_t *svn_err__temp = (svn_hash_write2(encoded_prophash, propstream, "PROPS-END", pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
...
ensure_appname(const char *progname, apr_pool_t *pool)
{
  ...
  return svn_dirent_basename(progname, ((void*)0));
}
...
usage(const char *progname, apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stderr, pool, dcgettext ("subversion", "Type '%s --help' for usage.\n", 5), progname)); if (svn_err__temp) return (svn_err__temp); } while (0);
...
help(const char *progname, apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_cmdline_printf( pool, dcgettext ("subversion", "usage: %s URL [-r LOWER[:UPPER]]\n\n" "Dump the contents of repository at remote URL " "to stdout in a 'dumpfile' portable format.\n" "Dump revisions LOWER rev through UPPER rev.\n" "LOWER defaults to 0 and UPPER defaults to the " "highest possible revision if omitted.\n\n" "Valid options:\n", 5), progname)); if (svn_err__temp) return (svn_err__temp); } while (0);
# 308 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svnrdump/svnrdump.c"
  for (i = 0; svnrdump__options[i].name && svnrdump__options[i].optch; i++)
    {
      const char *optstr;
      svn_opt_format_option(&optstr, svnrdump__options + i, (!0), pool);
      do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stdout, pool, "  %s\n", optstr)); if (svn_err__temp) return (svn_err__temp); } while (0);
    }
...
version(const char *progname, apr_pool_t *pool)
{
  ...
  return svn_opt_print_help3(((void*)0), progname, (!0), 0, ((void*)0),
                             ((void*)0), ((void*)0), ((void*)0), ((void*)0), ((void*)0), pool);
}
# 346 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svnrdump/svnrdump.c"
int
main(int argc, const char **argv)
{
  ...
  while (1)
    {
      ...
      switch(opt)
        ...
        case 'r':
          {
            revision_cut = (__extension__ (__builtin_constant_p (':') && !__builtin_constant_p (opt_arg) && (':') == '\0' ? (char *) __rawmemchr (opt_arg, ':') : __builtin_strchr (opt_arg, ':')));
            if (revision_cut)
              {
              ...
              start_revision = (svn_revnum_t)strtoul(opt_arg, ((void*)0), 10);
          }
          break;
        case 'q':
          quiet = (!0);
        ...
        case opt_version:
          ...
          break;
        case 'h':
          do { svn_error_t *svn_err__temp = (help(argv[0], pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svnrdump: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
          exit(0);
          break;
  ...
  do { svn_error_t *svn_err__temp = (svn_utf_cstring_to_utf8(&url, os->argv[os->ind], pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svnrdump: "); svn_error_clear(svn_err__temp); return 1; } } while (0);

  url = svn_uri_canonicalize(os->argv[os->ind], pool);


  do { svn_error_t *svn_err__temp = (open_connection(&session, url, non_interactive, username, password, config_dir, no_auth_cache, pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svnrdump: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
# 458 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/svnrdump/svnrdump.c"
  do { svn_error_t *svn_err__temp = (svn_ra_get_latest_revnum(session, &latest_revision, pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svnrdump: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
  ...
  if (end_revision > latest_revision)
    {
      do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stderr, pool, dcgettext ("subversion", "UPPER refers to a " "non-existent revision.\n", 5))); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svn: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
  ...
  if (end_revision < start_revision)
    {
      do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stderr, pool, dcgettext ("subversion", "LOWER cannot be greater " "than UPPER.\n", 5))); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svn: "); svn_error_clear(svn_err__temp); return 1; } } while (0);
  ...
  do { svn_error_t *svn_err__temp = (replay_range(session, url, start_revision, end_revision, quiet, pool)); if (svn_err__temp) { svn_handle_error2(svn_err__temp, stderr, 0, "svnrdump: "); svn_error_clear(svn_err__temp); return 1; } } while (0);

...
svn_dso_initialize2(void)
{
  ...
  status = apr_thread_mutex_create(&dso_mutex,
                                   0x0, dso_pool);
  if (status)
    return svn_error_wrap_apr(status, dcgettext ("subversion", "Can't create DSO mutex", 5));
...
svn_dso_load(apr_dso_handle_t **dso, const char *fname)
{
  ...
  if (status)
    return svn_error_wrap_apr(status, dcgettext ("subversion", "Can't grab DSO mutex", 5));
  ...
  if (*dso == ((void *) &not_there_sentinel))
    {
      *dso = ((void*)0);
      ...
      if (status)
        return svn_error_wrap_apr(status, dcgettext ("subversion", "Can't ungrab DSO mutex", 5));

...
svn_cmdline__getopt_init(apr_getopt_t **os,
                         ...
                         const char *argv[],
                         apr_pool_t *pool);
# 34 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
svn_path_cstring_to_utf8(const char **path_utf8,
                         const char *path_apr,
                         apr_pool_t *pool);
# 38 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
# 40 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
                                   ...
                                   apr_pool_t *pool)
{
  ...
  return srv;
# 75 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
}
# 84 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
static svn_error_t *
       ...
       apr_pool_t *pool)
{
  ...
  if (status)
    return (svn_error__locate("/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c",102), (svn_error_wrap_apr))(status, dcgettext ("subversion", "Can't open stdin", 5));

  if (! hide)
    {
      svn_boolean_t saw_first_half_of_eol = 0;
      do { svn_error_t *svn_err__temp = (svn_cmdline_fputs(prompt_msg, stderr, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
      fflush(stderr);
      ...
      while (1)
        {
          ...
          if (((status) == 4))
            continue;
          else if (status && status != ((20000 + 50000) + 23))
            return (svn_error__locate("/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c",120), (svn_error_wrap_apr))(status, dcgettext ("subversion", "Can't read stdin", 5));

          status = apr_file_getc(&c, fp);
          if (status)
            return (svn_error__locate("/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c",124), (svn_error_wrap_apr))(status, dcgettext ("subversion", "Can't read stdin", 5));
          ...
          if (saw_first_half_of_eol)
          ...
          else if (c == "\n"[0])
            {
              ...
              if (sizeof("\n") == 3)
                ...
                do { return (svn_error__malfunction( (!0), "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c", 147, ((void*)0))); } while (0);
            }
  ...
  else
    {
      ...
      size_t bufsize = 300;
      do { svn_error_t *svn_err__temp = (svn_cmdline_cstring_from_utf8(&prompt_stdout, prompt_msg, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

      svn_stringbuf_ensure(strbuf, bufsize);

      status = apr_password_get(prompt_stdout, strbuf->data, &bufsize);
      if (status)
        return (svn_error__locate("/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c",163), (svn_error_wrap_apr))(status, dcgettext ("subversion", "Can't get password", 5));
    }

  return svn_cmdline_cstring_to_utf8(result, strbuf->data, pool);
}
# 177 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
static svn_error_t *
maybe_print_realm(const char *realm, apr_pool_t *pool)
{
  if (realm)
    {
      do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stderr, pool, dcgettext ("subversion", "Authentication realm: %s\n", 5), realm)); if (svn_err__temp) return (svn_err__temp); } while (0);
                        ...
                        apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (svn_cmdline_fprintf(stderr, pool, prompt_text, realmstring, config_path)); if (svn_err__temp) return (svn_err__temp); } while (0);
...
# 463 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
...
# 492 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
  return plaintext_prompt_helper(may_save_plaintext, realmstring,
                                 prompt_string, prompt_text, baton,
                                 pool);
}

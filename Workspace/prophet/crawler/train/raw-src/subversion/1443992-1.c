# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 311 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
# 30 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
...
apr_status_t apr_password_get(const char *prompt, char *pwbuf,
                                           apr_size_t *bufsize);
# 31 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
# 1 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_poll.h" 1
# 24 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_poll.h"
...
# 25 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_poll.h" 2

# 1 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_inherit.h" 1
# 27 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_poll.h" 2
...
apr_status_t apr_temp_dir_get(const char **temp_dir,
                                           apr_pool_t *p);
# 28 "/home/ubuntu/Workspace/prophet/build/benchmarks/apache-deps/apr-build/include/apr-1/apr_poll.h" 2
...
apr_status_t apr_pollcb_poll(apr_pollcb_t *pollcb,
                                          ...
                                          void *baton);
# 32 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
# 33 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
# 35 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
svn_path_resolve_repos_relative_url(const char **absolute_url,
                                    ...
                                    apr_pool_t *pool);
# 39 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
svn_cmdline__edit_string_externally(svn_string_t **edited_contents,
                                    ...
                                    const char *encoding,
                                    apr_pool_t *pool);
# 41 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
# 42 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
...
# 47 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c" 2
                         ...
                         svn_boolean_t restore_state)
{
  ...
  if (restore_state && terminal->restore_state)
    tcsetattr(terminal->osinfd, 0, &terminal->attr);
...
{
  return terminal_cleanup_handler(baton, 0, (!0));
}
...
terminal_close(terminal_handle_t *terminal)
{
  ...
  status = terminal_cleanup_handler(terminal, (!0), (!0));
  if (status)
    return svn_error_create(status, ((void*)0), dcgettext ("subversion", "Can't close terminal", 5));
  return 0;
}
...
{
  apr_status_t status;
# 179 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
  apr_file_t *tmpfd;
  ...
  if (!status)
    {
      ...
      terminal_handle_init(terminal, tmpfd, tmpfd, 0, (!0), pool);
    }
  ...
  else
    {
      ...
      terminal_handle_init(terminal, infd, outfd, 0, 0, pool);
    }
      ...
      if (0 == tcgetattr(terminal->osinfd, &terminal->attr))
        {
          ...
          attr.c_lflag &= ~(0000001 | 0000002);
          attr.c_cc[6] = 1;
          attr.c_cc[5] = 0;
          if (noecho)
            attr.c_lflag &= ~(0000010);
          if (0 == tcsetattr(terminal->osinfd, 2, &attr))
            {
              terminal->noecho = noecho;
              terminal->restore_state = (!0);
            }
        }
    }
...
# 259 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
  status = apr_file_write_full(terminal->outfd, converted,
                               strlen(converted), ((void*)0));
  if (!status)
    status = apr_file_flush(terminal->outfd);
  if (status)
    return svn_error_wrap_apr(status, dcgettext ("subversion", "Can't write to terminal", 5));
  return 0;
}
# 278 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
static svn_error_t *
              ...
              svn_boolean_t can_erase, apr_pool_t *pool)
{
  apr_status_t status = 0;
  char ch;
# 354 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
  if (terminal->restore_state)
    {
      ...
      if (status)
        return svn_error_wrap_apr(status, dcgettext ("subversion", "Can't read from terminal", 5));

      if (ch == attr->c_cc[0] || ch == attr->c_cc[1])
        return svn_error_create(SVN_ERR_CANCELLED, ((void*)0), ((void*)0));
      else if (ch == '\r' || ch == '\n' || ch == attr->c_cc[11])
        *code = (0x80000 + 2);
      else if (ch == '\b'
               || ch == attr->c_cc[2] || ch == attr->c_cc[3])
        *code = (0x80000 + 1);
      else if (ch == attr->c_cc[4])
        *code = (0x80000 + 3);
      else if (!(((*__ctype_b_loc ())[(int) ((((unsigned char)(ch))))] & (unsigned short int) _IScntrl)))
...

  {
    ...
    pollset.p = pool;
    pollset.reqevents = 0x001;

    status = apr_poll(&pollset, 1, &n, -1);

    if (n == 1 && pollset.rtnevents & 0x001)
      status = 0;
  }
...
# 431 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
static svn_error_t *
       ...
       apr_pool_t *pool)
{
  ...
  svn_boolean_t saw_first_half_of_eol = 0;
  svn_stringbuf_t *strbuf = svn_stringbuf_create_empty(pool);
  terminal_handle_t terminal;
  int code;
  char c;

  do { svn_error_t *svn_err__temp = (terminal_open(&terminal, hide, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  do { svn_error_t *svn_err__temp = (terminal_puts(prompt_msg, &terminal, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

  while (1)
    {
      do { svn_error_t *svn_err__temp = (terminal_getc(&code, &terminal, (strbuf->len > 0), pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
      ...
      switch (code)
        ...
        case (0x80000 + 3):
          return svn_error_create(
              ...
              dcgettext ("subversion", "End of file while reading from terminal", 5));
      ...
      if (saw_first_half_of_eol)
      ...
      else if (c == "\n"[0])
        {
          ...
          if (sizeof("\n") == 3)
            ...
            do { return (svn_error__malfunction( (!0), "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c", 513, ((void*)0))); } while (0);
        }
  ...
  do { svn_error_t *svn_err__temp = (terminal_close(&terminal)); if (svn_err__temp) return (svn_err__temp); } while (0);

  return svn_cmdline_cstring_to_utf8(result, strbuf->data, pool);
}
# 538 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
static svn_error_t *
maybe_print_realm(const char *realm, apr_pool_t *pool)
{
  if (realm)
    {
      terminal_handle_t terminal;
      do { svn_error_t *svn_err__temp = (terminal_open(&terminal, 0, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
      do { svn_error_t *svn_err__temp = (terminal_puts( apr_psprintf(pool, dcgettext ("subversion", "Authentication realm: %s\n", 5), realm), &terminal, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
...
svn_cmdline_auth_simple_prompt(svn_auth_cred_simple_t **cred_p,
                               ...
                               apr_pool_t *pool)
{
  svn_auth_cred_simple_t *ret = memset(apr_palloc(pool, sizeof(*ret)), 0, sizeof(*ret));
  const char *pass_prompt;
  svn_cmdline_prompt_baton2_t *pb = baton;

  do { svn_error_t *svn_err__temp = (maybe_print_realm(realm, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

  if (username)
    ret->username = apr_pstrdup(pool, username);
  else
    do { svn_error_t *svn_err__temp = (prompt(&(ret->username), dcgettext ("subversion", "Username: ", 5), 0, pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

  pass_prompt = apr_psprintf(pool, dcgettext ("subversion", "Password for '%s': ", 5), ret->username);
  do { svn_error_t *svn_err__temp = (prompt(&(ret->password), pass_prompt, (!0), pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
...
svn_cmdline_auth_username_prompt(svn_auth_cred_username_t **cred_p,
                                 ...
                                 apr_pool_t *pool)
{
  svn_auth_cred_username_t *ret = memset(apr_palloc(pool, sizeof(*ret)), 0, sizeof(*ret));
  svn_cmdline_prompt_baton2_t *pb = baton;
   ...
   apr_pool_t *pool)
{
  ...
  svn_stringbuf_t *buf = svn_stringbuf_createf
    (pool, dcgettext ("subversion", "Error validating server certificate for '%s':\n", 5), realm);

  if (failures & 0x00000008)
    {
      svn_stringbuf_appendcstr
        (buf,
         dcgettext ("subversion", " - The certificate is not issued by a trusted authority. Use the\n" "   fingerprint to validate the certificate manually!\n", 5));
  ...
  if (failures & 0x00000004)
    {
      svn_stringbuf_appendcstr
        (buf, dcgettext ("subversion", " - The certificate hostname does not match.\n", 5));
    }

  if (failures & 0x00000001)
    {
      svn_stringbuf_appendcstr
        (buf, dcgettext ("subversion", " - The certificate is not yet valid.\n", 5));
    }

  if (failures & 0x00000002)
    {
      svn_stringbuf_appendcstr
        (buf, dcgettext ("subversion", " - The certificate has expired.\n", 5));
    }

  if (failures & 0x40000000)
    {
      svn_stringbuf_appendcstr
        (buf, dcgettext ("subversion", " - The certificate has an unknown error.\n", 5));
    }

  msg = svn_stringbuf_createf
    (pool,
     dcgettext ("subversion", "Certificate information:\n" " - Hostname: %s\n" " - Valid: from %s until %s\n" " - Issuer: %s\n" " - Fingerprint: %s\n", 5),
  ...
  if (may_save)
    {
      svn_stringbuf_appendcstr
        (buf, dcgettext ("subversion", "(R)eject, accept (t)emporarily or accept (p)ermanently? ", 5));
    }
  else
    {
      svn_stringbuf_appendcstr(buf, dcgettext ("subversion", "(R)eject or accept (t)emporarily? ", 5));
    }
  do { svn_error_t *svn_err__temp = (prompt(&choice, buf->data, 0, pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
   ...
   apr_pool_t *pool)
{
  ...
  do { svn_error_t *svn_err__temp = (maybe_print_realm(realm, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  do { svn_error_t *svn_err__temp = (prompt(&cert_file, dcgettext ("subversion", "Client certificate filename: ", 5), 0, pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

  do { svn_error_t *svn_err__temp = (svn_dirent_get_absolute(&abs_cert_file, cert_file, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
   ...
   apr_pool_t *pool)
{
  ...
  const char *result;
  const char *text = apr_psprintf(pool, dcgettext ("subversion", "Passphrase for '%s': ", 5), realm);
  svn_cmdline_prompt_baton2_t *pb = baton;

  do { svn_error_t *svn_err__temp = (prompt(&result, text, (!0), pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

  cred = memset(apr_palloc(pool, sizeof(*cred)), 0, sizeof(*cred));
                        ...
                        apr_pool_t *pool)
{
  ...
  svn_boolean_t answered = 0;
  svn_cmdline_prompt_baton2_t *pb = baton;
  const char *config_path = ((void*)0);
  terminal_handle_t terminal;

  if (pb)
    do { svn_error_t *svn_err__temp = (svn_config_get_user_config_path(&config_path, pb->config_dir, "servers", pool)); if (svn_err__temp) return (svn_err__temp); } while (0);


  do { svn_error_t *svn_err__temp = (terminal_open(&terminal, 0, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  do { svn_error_t *svn_err__temp = (terminal_puts(apr_psprintf(pool, prompt_text, realmstring, config_path), &terminal, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);
  ...
  do
    {
      svn_error_t *err = prompt(&answer, prompt_string, 0, pb, pool);
      if (err)
        {
          if (err->apr_err == SVN_ERR_CANCELLED)
            {
              svn_error_clear(err);
              *may_save_plaintext = 0;
              return 0;
            }
          else
            return err;
        }
      if (apr_strnatcasecmp(answer, dcgettext ("subversion", "yes", 5)) == 0 ||
          apr_strnatcasecmp(answer, dcgettext ("subversion", "y", 5)) == 0)
        {
          *may_save_plaintext = (!0);
          answered = (!0);
        }
      else if (apr_strnatcasecmp(answer, dcgettext ("subversion", "no", 5)) == 0 ||
               apr_strnatcasecmp(answer, dcgettext ("subversion", "n", 5)) == 0)
        {
          *may_save_plaintext = 0;
          answered = (!0);
        }
      else
          prompt_string = dcgettext ("subversion", "Please type 'yes' or 'no': ", 5);
    }
  while (! answered);
...
svn_cmdline_auth_plaintext_prompt(svn_boolean_t *may_save_plaintext,
                                  ...
                                  apr_pool_t *pool)
{
  const char *prompt_string = dcgettext ("subversion", "Store password unencrypted (yes/no)? ", 5);
  const char *prompt_text =
  dcgettext ("subversion", "\n-----------------------------------------------------------------------" "\nATTENTION!  Your password for authentication realm:\n" "\n" "   %s\n" "\n" "can only be stored to disk unencrypted!  You are advised to configure\n" "your system so that Subversion can store passwords encrypted, if\n" "possible.  See the documentation for details.\n" "\n" "You can avoid future appearances of this warning by setting the value\n" "of the 'store-plaintext-passwords' option to either 'yes' or 'no' in\n" "'%s'.\n" "-----------------------------------------------------------------------\n", 5);
# 834 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
...
svn_cmdline_auth_plaintext_passphrase_prompt(svn_boolean_t *may_save_plaintext,
                                             ...
                                             apr_pool_t *pool)
{
  const char *prompt_string = dcgettext ("subversion", "Store passphrase unencrypted (yes/no)? ", 5);
  const char *prompt_text =
  dcgettext ("subversion", "\n-----------------------------------------------------------------------\n" "ATTENTION!  Your passphrase for client certificate:\n" "\n" "   %s\n" "\n" "can only be stored to disk unencrypted!  You are advised to configure\n" "your system so that Subversion can store passphrase encrypted, if\n" "possible.  See the documentation for details.\n" "\n" "You can avoid future appearances of this warning by setting the value\n" "of the 'store-ssl-client-cert-pp-plaintext' option to either 'yes' or\n" "'no' in '%s'.\n" "-----------------------------------------------------------------------\n", 5);
# 863 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/subversion/libsvn_subr/prompt.c"
  return plaintext_prompt_helper(may_save_plaintext, realmstring,
                                 prompt_string, prompt_text, baton,
                                 pool);
}
                         ...
                         apr_pool_t *pool)
{
  ...
  return prompt(result, prompt_str, 0 ,
                                              ...
                                              apr_pool_t *pool)
{
  ...
  pass_prompt = apr_psprintf(pool, dcgettext ("subversion", "Password for '%s' GNOME keyring: ", 5),
                             keyring_name);
  do { svn_error_t *svn_err__temp = (prompt(&password, pass_prompt, (!0), pb, pool)); if (svn_err__temp) return (svn_err__temp); } while (0);

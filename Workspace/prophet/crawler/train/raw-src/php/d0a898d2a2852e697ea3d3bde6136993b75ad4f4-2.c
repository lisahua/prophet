# 1 "ext/date/lib/unixtime2tm.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "ext/date/lib/unixtime2tm.c" 2
# 21 "ext/date/lib/unixtime2tm.c"
...
# 22 "ext/date/lib/unixtime2tm.c" 2
# 35 "ext/date/lib/unixtime2tm.c"
...
void timelib_unixtime2gmt(timelib_time* tm, timelib_sll ts)
{
 ...
 days = ts / 86400;
 remainder = ts - (days * 86400);
 if (ts < 0 && remainder == 0) {
  days++;
  remainder -= 86400;
 }
 if (0) { printf("days=%lld, rem=%lld\n", days, remainder); };

 if (ts >= 0) {
  tmp_days = days + 1;

  if (tmp_days >= 146097 || tmp_days <= -146097) {
   cur_year += 400 * (tmp_days / 146097);
   tmp_days -= 146097 * (tmp_days / 146097);
  }
  ...
  tmp_days = days;
# 84 "ext/date/lib/unixtime2tm.c"
  while (tmp_days <= 0) {
   if (tmp_days < -1460970) {
    cur_year -= 4000;
    if (0) { printf("tmp_days=%lld, year=%lld\n", tmp_days, cur_year); };
    tmp_days += 1460970;
   } else {
    cur_year--;
    if (0) { printf("tmp_days=%lld, year=%lld\n", tmp_days, cur_year); };
 ...
 months = ((cur_year) % 4 == 0 && ((cur_year) % 100 != 0 || (cur_year) % 400 == 0)) ? month_tab_leap : month_tab;
 if (((cur_year) % 4 == 0 && ((cur_year) % 100 != 0 || (cur_year) % 400 == 0)) && cur_year < 1970) {
  tmp_days--;
 }
 i = 11;
 while (i > 0) {
  if (0) { printf("month=%lld (%d)\n", i, months[i]); };
  if (tmp_days > months[i]) {
   break;
  }
  i--;
 }
 if (0) { printf("A: ts=%lld, year=%lld, month=%lld, day=%lld,", ts, cur_year, i + 1, tmp_days - months[i]); };


 hours = remainder / 3600;
 minutes = (remainder - hours * 3600) / 60;
 seconds = remainder % 60;
 if (0) { printf(" hour=%lld, minute=%lld, second=%lld\n", hours, minutes, seconds); };
...
void timelib_set_timezone(timelib_time *t, timelib_tzinfo *tz)
{
 ...
 t->tz_abbr = (__extension__ (__builtin_constant_p (gmt_offset->abbr) && ((size_t)(const void *)((gmt_offset->abbr) + 1) - (size_t)(const void *)(gmt_offset->abbr) == 1) ? (((const char *) (gmt_offset->abbr))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (gmt_offset->abbr) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, gmt_offset->abbr, __len); __retval; })) : __strdup (gmt_offset->abbr)));
 timelib_time_offset_dtor(gmt_offset);
...
int timelib_apply_localtime(timelib_time *t, unsigned int localtime)
{
 if (localtime) {
  ...
  if (0) { printf("Converting from GMT time to local time\n"); };


  if (!t->tz_info) {
   if (0) { printf("E: No timezone configured, can't switch to local time\n"); };
   return -1;
  }
  ...
  if (0) { printf("Converting from local time to GMT time\n"); };

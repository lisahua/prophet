...
# 59 "../lib/curlx.h" 2
# 27 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
                  ...
                  FILE *error)
{
  ...
  *glob = ((void*)0);
  ...
  if(!glob_expand) {
    do { free((glob_buffer)); (glob_buffer) = ((void*)0);} while(0);
    return CURLE_OUT_OF_MEMORY;
  }
  ...
  for(i = 0; i < glob->size; i++) {
    if((glob->pattern[i].type == UPTSet) &&
      ...
      for(elem = glob->pattern[i].content.Set.size - 1;
          elem >= 0;
          --elem) {
        do { free((glob->pattern[i].content.Set.elements[elem])); (glob->pattern[i].content.Set.elements[elem]) = ((void*)0);} while(0);
      }
      do { free((glob->pattern[i].content.Set.elements)); (glob->pattern[i].content.Set.elements) = ((void*)0);} while(0);
    }
  }
  do { free((glob->glob_buffer)); (glob->glob_buffer) = ((void*)0);} while(0);
  do { free((glob)); (glob) = ((void*)0);} while(0);
}
      ...
      if(pat) {
        ...
        default:
          curl_mfprintf(stderr, "internal error: invalid pattern type (%d)\n",
                  (int)pat->type);
          do { free((target)); (target) = ((void*)0);} while(0);
          return CURLE_FAILED_INIT;
        }
      }
      else {

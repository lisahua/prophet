# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
# 23 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
...
# 24 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
# 26 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
# 31 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
# 34 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
void Curl_ssl_md5sum(unsigned char *tmp,
                     ...
                     size_t md5len);
# 36 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
# 1 "../lib/strequal.h" 1
# 37 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
# 38 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
# 1 "../lib/sendf.h" 1
# 27 "../lib/sendf.h"
...
int Curl_debug(struct SessionHandle *handle, curl_infotype type,
               ...
               struct connectdata *conn);
# 39 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
...
# 42 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2


# 1 "../lib/memdebug.h" 1
# 45 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c" 2
# 55 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static size_t readfromfile(struct Form *form, char *buffer, size_t size);
static char *formboundary(struct SessionHandle *data);
# 75 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
...
{
  struct curl_httppost *post;
  post = Curl_ccalloc(1, sizeof(struct curl_httppost));
  if(post) {
    post->name = name;
...
# 134 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static FormInfo * AddFormInfo(char *value,
                              char *contenttype,
                              FormInfo *parent_form_info)
{
  FormInfo *form_info;
  form_info = Curl_ccalloc(1, sizeof(struct FormInfo));
  if(form_info) {
    if(value)
...
# 171 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static const char * ContentTypeForFilename (const char *filename,
                                            const char *prevtype)
{
  const char *contenttype = ((void*)0);
  unsigned int i;
  ...
  if(filename) {
    for(i=0; i<sizeof(ctts)/sizeof(ctts[0]); i++) {
      if(strlen(filename) >= strlen(ctts[i].extension)) {
        if(curl_strequal(filename + strlen(filename) - strlen(ctts[i].extension),ctts[i].extension)) {
...
# 226 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static char *memdup(const char *src, size_t buffer_length)
{
  ...
  buffer = Curl_cmalloc(length+add);
  if(!buffer)
    return ((void*)0);
...
# 304 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static
CURLFORMcode FormAdd(struct curl_httppost **httppost,
                     struct curl_httppost **last_post,
                     va_list params)
{
  FormInfo *first_form, *current_form, *form = ((void*)0);
  CURLFORMcode return_value = CURL_FORMADD_OK;
  const char *prevtype = ((void*)0);
  struct curl_httppost *post = ((void*)0);
  CURLformoption option;
  struct curl_forms *forms = ((void*)0);
  char *array_value=((void*)0);
  ...
  first_form = Curl_ccalloc(1, sizeof(struct FormInfo));
  if(!first_form)
  ...
  while(return_value == CURL_FORMADD_OK) {
    ...
    if(array_state && forms) {
      ...
      option = __builtin_va_arg(params, CURLformoption);
      if(CURLFORM_END == option)
        break;
    }
    ...
    case CURLFORM_ARRAY:
      if(array_state)
        ...
        forms = __builtin_va_arg(params, struct curl_forms *);
        if(forms)
          array_state = 1;
        else
    ...
    case CURLFORM_COPYNAME:
      if(current_form->name)
        ...
        char *name = array_state?
          array_value:__builtin_va_arg(params, char *);
        if(name)
    ...
    case CURLFORM_NAMELENGTH:
      if(current_form->namelength)
        ...
        current_form->namelength =
          array_state?(size_t)array_value:(size_t)__builtin_va_arg(params, long);
      break;
    ...
    case CURLFORM_PTRCONTENTS:
      current_form->flags |= (1<<3);
    case CURLFORM_COPYCONTENTS:
      if(current_form->value)
        return_value = CURL_FORMADD_OPTION_TWICE;
      else {
        char *value =
          array_state?array_value:__builtin_va_arg(params, char *);
        if(value)
    ...
    case CURLFORM_FILECONTENT:
      if(current_form->flags & ((1<<3)|(1<<1)))
        return_value = CURL_FORMADD_OPTION_TWICE;
      else {
        const char *filename = array_state?
          array_value:__builtin_va_arg(params, char *);
        if(filename) {
          current_form->value = Curl_cstrdup(filename);
          if(!current_form->value)
            return_value = CURL_FORMADD_MEMORY;
          else {
            current_form->flags |= (1<<1);
            current_form->value_alloc = 1;
          }
        }
        else
          return_value = CURL_FORMADD_NULL;
    ...
    case CURLFORM_FILE:
      {
        const char *filename = array_state?array_value:
          __builtin_va_arg(params, char *);

        if(current_form->value) {
          if(current_form->flags & (1<<0)) {
            if(filename) {
              char *fname = Curl_cstrdup(filename);
              if(!fname)
                return_value = CURL_FORMADD_MEMORY;
              else {
                form = AddFormInfo(fname, ((void*)0), current_form);
                if(!form) {
                  do {if((fname)) {Curl_cfree((fname)); (fname) = ((void*)0);}} while(0);
                  return_value = CURL_FORMADD_MEMORY;
                }
                else {
                  form->value_alloc = 1;
    ...
    case CURLFORM_CONTENTTYPE:
      {
        ...
        if(current_form->contenttype) {
          if(current_form->flags & (1<<0)) {
            if(contenttype) {
              char *type = Curl_cstrdup(contenttype);
              if(!type)
                return_value = CURL_FORMADD_MEMORY;
              else {
                form = AddFormInfo(((void*)0), type, current_form);
                if(!form) {
                  do {if((type)) {Curl_cfree((type)); (type) = ((void*)0);}} while(0);
                  return_value = CURL_FORMADD_MEMORY;
                }
                else {
                  form->contenttype_alloc = 1;
                  current_form = form;
                  form = ((void*)0);
                }
              }
          ...
          if(contenttype) {
            current_form->contenttype = Curl_cstrdup(contenttype);
            if(!current_form->contenttype)
              return_value = CURL_FORMADD_MEMORY;
            else
              current_form->contenttype_alloc = 1;
          }
          else
            return_value = CURL_FORMADD_NULL;
        }
    ...
    case CURLFORM_CONTENTHEADER:
      {
        ...
        struct curl_slist* list = array_state?
          ...
          __builtin_va_arg(params, struct curl_slist*);
    ...
    case CURLFORM_BUFFER:
      {
        ...
        if(current_form->showfilename)
          ...
          current_form->showfilename = Curl_cstrdup(filename);
          if(!current_form->showfilename)
            return_value = CURL_FORMADD_MEMORY;
          else
            current_form->showfilename_alloc = 1;
        }
        break;
      }
    default:
  ...
  if(CURL_FORMADD_OK != return_value) {
    ...
    FormInfo *ptr;
    for(ptr = first_form; ptr != ((void*)0); ptr = ptr->more) {
      if(ptr->name_alloc) {
        do {if((ptr->name)) {Curl_cfree((ptr->name)); (ptr->name) = ((void*)0);}} while(0);
        ptr->name_alloc = 0;
      }
      if(ptr->value_alloc) {
        do {if((ptr->value)) {Curl_cfree((ptr->value)); (ptr->value) = ((void*)0);}} while(0);
        ptr->value_alloc = 0;
      }
      if(ptr->contenttype_alloc) {
        do {if((ptr->contenttype)) {Curl_cfree((ptr->contenttype)); (ptr->contenttype) = ((void*)0);}} while(0);
        ptr->contenttype_alloc = 0;
      }
      if(ptr->showfilename_alloc) {
        do {if((ptr->showfilename)) {Curl_cfree((ptr->showfilename)); (ptr->showfilename) = ((void*)0);}} while(0);
        ptr->showfilename_alloc = 0;
      }
    }
  }
  ...
  if(CURL_FORMADD_OK == return_value) {
    ...
    for(form = first_form;
      ...
      if(((!form->name || !form->value) && !post) ||
        ...
        if(((form->flags & (1<<0)) ||
          ...
          form->contenttype
            = Curl_cstrdup(ContentTypeForFilename(form->value, prevtype));
          if(!form->contenttype) {
            return_value = CURL_FORMADD_MEMORY;
            break;
          }
    ...
    if(CURL_FORMADD_OK != return_value) {
      ...
      FormInfo *ptr;
      for(ptr = form; ptr != ((void*)0); ptr = ptr->more) {
        if(ptr->name_alloc) {
          do {if((ptr->name)) {Curl_cfree((ptr->name)); (ptr->name) = ((void*)0);}} while(0);
          ptr->name_alloc = 0;
        }
  ...
  while(first_form) {
    ...
    do {if((first_form)) {Curl_cfree((first_form)); (first_form) = ((void*)0);}} while(0);
    first_form = ptr;
  }
...
# 846 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static CURLcode AddFormData(struct FormData **formp,
                            ...
                            curl_off_t *size)
{
  struct FormData *newform = Curl_cmalloc(sizeof(struct FormData));
  if(!newform)
  ...
  if(type <= FORM_CONTENT) {
    ...
    newform->line = Curl_cmalloc(length+1);
    if(!newform->line) {
      Curl_cfree(newform);
      return CURLE_OUT_OF_MEMORY;
    }
  ...
  if(size) {
    if(type != FORM_FILE)
      ...
      *size += length;
      ...
      if(!curl_strequal("-",newform->line)) {
        struct stat file;
        if(!stat(newform->line, &file) && !((((file.st_mode)) & 0170000) == (0040000)))
          *size += (file.st_size);
        else
                             ...
                             const char *fmt, ...)
{
  ...
  curl_mvsnprintf(s, sizeof(s), fmt, ap);
  __builtin_va_end(ap);
...
void Curl_formclean(struct FormData **form_ptr)
{
  ...
  do {
    ...
    if(form->type <= FORM_CONTENT)
      Curl_cfree(form->line);
    Curl_cfree(form);

  } while((form = next) != ((void*)0));

  *form_ptr = ((void*)0);
}
# 952 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
                 ...
                 curl_formget_callback append)
{
  ...
  rc = Curl_getformdata(((void*)0), &data, form, ((void*)0), &size);
  if(rc != CURLE_OK)
    return (int)rc;
...
# 1075 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static char *strippath(const char *fullfile)
{
  char *filename;
  char *base;
  filename = Curl_cstrdup(fullfile);

  if(!filename)
    return ((void*)0);
  base = Curl_cstrdup(__xpg_basename(filename));
                                      ...
                                      curl_off_t *size)
{
  ...
  char *filebasename = ((void*)0);
  char *filename_escaped = ((void*)0);
  ...
  if((__extension__ (__builtin_constant_p ('\\') && !__builtin_constant_p (filename) && ('\\') == '\0' ? (char *) __rawmemchr (filename, '\\') : __builtin_strchr (filename, '\\'))) || (__extension__ (__builtin_constant_p ('"') && !__builtin_constant_p (filename) && ('"') == '\0' ? (char *) __rawmemchr (filename, '"') : __builtin_strchr (filename, '"')))) {
    char *p0, *p1;


    filename_escaped = Curl_cmalloc(strlen(filename)*2+1);
    if(!filename_escaped)
      return CURLE_OUT_OF_MEMORY;
    p0 = filename_escaped;
    p1 = filename;
  ...
  result = AddFormDataf(form, size,
                        "; filename=\"%s\"",
                        filename);
  do {if((filename_escaped)) {Curl_cfree((filename_escaped)); (filename_escaped) = ((void*)0);}} while(0);
  do {if((filebasename)) {Curl_cfree((filebasename)); (filebasename) = ((void*)0);}} while(0);
  return result;
}
# 1144 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
CURLcode Curl_getformdata(struct SessionHandle *data,
                          ...
                          curl_off_t *sizep)
{
  ...
  *finalform = ((void*)0);
  ...
  result = AddFormDataf(&form, ((void*)0),
                        "%s; boundary=%s\r\n",
  ...
  if(result) {
    do {if((boundary)) {Curl_cfree((boundary)); (boundary) = ((void*)0);}} while(0);
    return result;
  }
  ...
  do {
    ...
    if(post->more) {
      ...
      do {if((fileboundary)) {Curl_cfree((fileboundary)); (fileboundary) = ((void*)0);}} while(0);
    ...
    do {
      ...
      else if(post->flags & ((1<<0)|(1<<4)|
                             (1<<6))) {



        if(post->showfilename || (post->flags & (1<<0))) {
      ...
      if((post->flags & (1<<0)) ||
        ...
        fileread = curl_strequal("-",file->contents)?
          stdin:fopen(file->contents, "rb");
        ...
        if(fileread) {
          ...
          if(data)
            Curl_failf(data, "couldn't open file \"%s\"", file->contents);
          *finalform = ((void*)0);
          result = CURLE_READ_ERROR;
        }
      }
      else if(post->flags & (1<<4))

        result = AddFormData(&form, FORM_CONTENT, post->buffer,
                             post->bufferlength, &size);
      else if(post->flags & (1<<6))
...
# 1444 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/formdata.c"
static size_t readfromfile(struct Form *form, char *buffer,
                           size_t size)
{
  size_t nread;
  _Bool callback = (form->data->type == FORM_CALLBACK)?1:0;

  if(callback) {
    if(form->fread_func == 0)
      return 0;
    else
    ...
    if(!form->fp) {
      ...
      form->fp = fopen(form->data->line, "rb");
      if(!form->fp)
        return (size_t)-1;
    }

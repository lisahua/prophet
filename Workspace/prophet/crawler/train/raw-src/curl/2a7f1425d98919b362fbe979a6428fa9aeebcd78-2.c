...
static GlobCode glob_fixed(URLGlob *glob, unsigned long *amount)
{
  ...
  if(!pat->content.Set.elements) {
    curl_msnprintf(glob->errormsg, sizeof(glob->errormsg), "out of memory\n");
    return GLOB_NO_MEM;
  }
  pat->content.Set.elements[0] = (__extension__ (__builtin_constant_p (glob->glob_buffer) && ((size_t)(const void *)((glob->glob_buffer) + 1) - (size_t)(const void *)(glob->glob_buffer) == 1) ? (((const char *) (glob->glob_buffer))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (glob->glob_buffer) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, glob->glob_buffer, __len); __retval; })) : __strdup (glob->glob_buffer)));
                         ...
                         int globindex)
{
  ...
  while(!done) {
    switch (*pattern) {
    case '\0':
      curl_msnprintf(glob->errormsg, sizeof(glob->errormsg),

# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 23 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
...
# 24 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 37 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
# 1 "../lib/smb.h" 1
# 25 "../lib/smb.h"
enum smb_conn_state {
  SMB_NOT_CONNECTED = 0,
...
# 38 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/urldata.h" 1
# 82 "../lib/urldata.h"
# 1 "../lib/cookie.h" 1
# 26 "../lib/cookie.h"
...
# 39 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/sendf.h" 1
# 27 "../lib/sendf.h"
...
int Curl_debug(struct SessionHandle *handle, curl_infotype type,
               ...
               struct connectdata *conn);
# 40 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/multiif.h" 1
# 28 "../lib/multiif.h"
...
# 41 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/connect.h" 1
# 26 "../lib/connect.h"
# 1 "../lib/nonblock.h" 1
# 27 "../lib/nonblock.h"
...
CURLcode Curl_socket(struct connectdata *conn,
                     ...
                     curl_socket_t *sockfd);
# 42 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/progress.h" 1
# 28 "../lib/progress.h"
typedef enum {
  TIMER_NONE,
...
# 43 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/transfer.h" 1
# 25 "../lib/transfer.h"
...
long Curl_sleep_time(curl_off_t rate_bps, curl_off_t cur_rate_bps,
                     int pkt_size);
# 44 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/vtls/vtls.h" 1
# 26 "../lib/vtls/vtls.h"
...
# 45 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/curl_ntlm_core.h" 1
# 63 "../lib/curl_ntlm_core.h"
...
# 46 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
...
# 47 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2
# 1 "../lib/escape.h" 1
# 27 "../lib/escape.h"
CURLcode Curl_urldecode(struct SessionHandle *data,
                        ...
                        _Bool reject_crlf);
# 48 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2


# 1 "../lib/memdebug.h" 1
# 51 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c" 2


static CURLcode smb_setup_connection(struct connectdata *conn);
static CURLcode smb_connect(struct connectdata *conn, _Bool *done);
static CURLcode smb_connection_state(struct connectdata *conn, _Bool *done);
static CURLcode smb_request_state(struct connectdata *conn, _Bool *done);
static CURLcode smb_done(struct connectdata *conn, CURLcode status,
                         _Bool premature);
static CURLcode smb_disconnect(struct connectdata *conn, _Bool dead);
...
# 159 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
enum smb_req_state {
  SMB_REQUESTING,
  SMB_TREE_CONNECT,
  SMB_OPEN,
...
# 199 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
  smb->state = newstate;
}
...
# 225 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/smb.c"
  req->state = newstate;
}
...
static CURLcode smb_setup_connection(struct connectdata *conn)
{
  ...
  conn->data->req.protop = req = Curl_ccalloc(1, sizeof(struct smb_request));
  if(!req)
...
static CURLcode smb_connect(struct connectdata *conn, _Bool *done)
{
  ...
  smbc->send_buf = Curl_cmalloc((0x8000 + 0x1000));
  smbc->recv_buf = Curl_cmalloc((0x8000 + 0x1000));
  ...
  slash = (__extension__ (__builtin_constant_p ('/') && !__builtin_constant_p (conn->user) && ('/') == '\0' ? (char *) __rawmemchr (conn->user, '/') : __builtin_strchr (conn->user, '/')));
  if(!slash)
    slash = (__extension__ (__builtin_constant_p ('\\') && !__builtin_constant_p (conn->user) && ('\\') == '\0' ? (char *) __rawmemchr (conn->user, '\\') : __builtin_strchr (conn->user, '\\')));

  if(slash) {
    smbc->user = slash + 1;
    smbc->domain = Curl_cstrdup(conn->user);
    if(!smbc->domain)
      return CURLE_OUT_OF_MEMORY;
    ...
    smbc->user = conn->user;
    smbc->domain = Curl_cstrdup(conn->host.name);
    if(!smbc->domain)
      return CURLE_OUT_OF_MEMORY;
  }
...
static CURLcode smb_recv_message(struct connectdata *conn, void **msg)
{
  ...
  size_t nbt_size;
  size_t msg_size;
  size_t len = (0x8000 + 0x1000) - smbc->got;
  CURLcode result;

  result = Curl_read(conn, 0, buf + smbc->got, len, &bytes_read);
  if(result)
    return result;
  ...
  nbt_size = (__extension__ ({ unsigned short int __v, __x = (unsigned short int) (*(unsigned short *)(buf + sizeof(unsigned short))); if (__builtin_constant_p (__x)) __v = ((unsigned short int) ((((__x) >> 8) & 0xff) | (((__x) & 0xff) << 8))); else __asm__ ("rorw $8, %w0" : "=r" (__v) : "0" (__x) : "cc"); __v; })) +
                   sizeof(unsigned int);
                               ...
                               unsigned char cmd, size_t len)
{
  ...
  memset(h, 0, sizeof(*h));
  h->nbt_length = (__extension__ ({ unsigned short int __v, __x = (unsigned short int) ((unsigned short) (sizeof(*h) - sizeof(unsigned int) + len)); if (__builtin_constant_p (__x)) __v = ((unsigned short int) ((((__x) >> 8) & 0xff) | (((__x) & 0xff) << 8))); else __asm__ ("rorw $8, %w0" : "=r" (__v) : "0" (__x) : "cc"); __v; }));

  memcpy((char *)h->magic, "\xffSMB", 4);
  h->command = cmd;
  h->flags = 0x10 | 0x08;
  h->flags2 = (0x0040 | 0x0001);
  h->uid = (smbc->uid);
  h->tid = (req->tid);
  pid = getpid();
  h->pid_high = ((unsigned short)(pid >> 16));
  h->pid = ((unsigned short) pid);
}
...
static CURLcode smb_send(struct connectdata *conn, ssize_t len)
{
  ...
  result = Curl_write(conn, 0, smbc->send_buf, len, &bytes_written);
  if(result)
    return result;
...
static CURLcode smb_flush(struct connectdata *conn)
{
  ...
  result = Curl_write(conn, 0, smbc->send_buf + smbc->sent,
                      len, &bytes_written);
  if(result)
    return result;
...
static CURLcode smb_send_setup(struct connectdata *conn)
{
  ...
  byte_count += strlen("x86_64-unknown-linux-gnu") + strlen("curl") + 4;
  ...
  msg.max_buffer_size = ((0x8000 + 0x1000));
  msg.max_mpx_count = (1);
  msg.vc_number = (1);
  msg.session_key = (smbc->session_key);
  msg.capabilities = (0x08);
  msg.lengths[0] = (sizeof(lm));
  msg.lengths[1] = (sizeof(nt));
  ...
  strcpy(p, ("x86_64-unknown-linux-gnu")); p += strlen("x86_64-unknown-linux-gnu") + 1;;
  strcpy(p, ("curl")); p += strlen("curl") + 1;;
  byte_count = p - msg.bytes;
  msg.byte_count = ((unsigned short)byte_count);

  return smb_send_message(conn, 0x73, &msg,
...
static CURLcode smb_send_tree_connect(struct connectdata *conn)
{
  ...
  return smb_send_message(conn, 0x75, &msg,
                          sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_open(struct connectdata *conn)
{
  ...
  msg.name_length = ((unsigned short)byte_count);
  msg.share_access = (0x07);
  if(conn->data->set.upload) {
    msg.access = (0x80000000 | 0x40000000);
    msg.create_disposition = (0x05);
  }
  else {
    msg.access = (0x80000000);
    msg.create_disposition = (0x01);
  }
  msg.byte_count = ((unsigned short) ++byte_count);
  strcpy(msg.bytes, req->path);

  return smb_send_message(conn, 0xa2, &msg,
                          sizeof(msg) - sizeof(msg.bytes) + byte_count);
}
...
static CURLcode smb_send_close(struct connectdata *conn)
{
  ...
  return smb_send_message(conn, 0x04, &msg, sizeof(msg));
}
...
static CURLcode smb_send_tree_disconnect(struct connectdata *conn)
{
  ...
  return smb_send_message(conn, 0x71, &msg, sizeof(msg));
}
...
static CURLcode smb_send_read(struct connectdata *conn)
{
  ...
  msg.andx.command = 0xff;
  msg.fid = (req->fid);
  msg.offset = ((unsigned int) offset);
  msg.offset_high = ((unsigned int) (offset >> 32));
  msg.min_bytes = (0x8000);
  msg.max_bytes = (0x8000);

  return smb_send_message(conn, 0x2e, &msg, sizeof(msg));
}
...
static CURLcode smb_send_write(struct connectdata *conn)
{
  ...
  msg->offset = ((unsigned int) offset);
  msg->offset_high = ((unsigned int) (offset >> 32));
  msg->data_length = ((unsigned short) nread);
  msg->data_offset = (sizeof(*msg) - sizeof(unsigned int));

  smb_format_message(conn, &msg->h, 0x2f,
...
static CURLcode smb_connection_state(struct connectdata *conn, _Bool *done)
{
  ...
  if(smbc->state == SMB_CONNECTING) {
    ...
    if((conn->handler->flags & (1<<0))) {
      _Bool ssl_done;
      result = Curl_ssl_connect_nonblocking(conn, 0, &ssl_done);
      if(result && result != CURLE_AGAIN)
        return result;
      if(!ssl_done)
        return CURLE_OK;
...
static CURLcode smb_request_state(struct connectdata *conn, _Bool *done)
{
  ...
  case SMB_OPEN:
    ...
    if(conn->data->set.upload) {
      ...
      conn->data->req.size =
        (((struct smb_nt_create_response *)msg)->end_of_file);
  ...
  case SMB_DOWNLOAD:
    ...
    len = (*(unsigned short *)((char *)msg + sizeof(struct smb_header) + 11));

    off = (*(unsigned short *)((char *)msg + sizeof(struct smb_header) + 13));
    ...
    next_state = (len < 0x8000) ? SMB_CLOSE : SMB_DOWNLOAD;
    break;
  ...
  case SMB_UPLOAD:
    ...
    len = (*(unsigned short *)((char *)msg + sizeof(struct smb_header) + 5));
                         ...
                         _Bool premature)
{
  ...
  do {if((req->share)) {Curl_cfree((req->share)); (req->share) = ((void*)0);}} while(0);
...
static CURLcode smb_disconnect(struct connectdata *conn, _Bool dead)
{
  ...
  do {if((smbc->domain)) {Curl_cfree((smbc->domain)); (smbc->domain) = ((void*)0);}} while(0);
  do {if((smbc->send_buf)) {Curl_cfree((smbc->send_buf)); (smbc->send_buf) = ((void*)0);}} while(0);
  do {if((smbc->recv_buf)) {Curl_cfree((smbc->recv_buf)); (smbc->recv_buf) = ((void*)0);}} while(0);
...
static CURLcode smb_parse_url_path(struct connectdata *conn)
{
  ...
  result = Curl_urldecode(data, data->state.path, 0, &path, ((void*)0), 1);
  if(result)
    return result;


  req->share = Curl_cstrdup((*path == '/' || *path == '\\') ? path + 1 : path);
  if(!req->share) {
    do {if((path)) {Curl_cfree((path)); (path) = ((void*)0);}} while(0);

    return CURLE_OUT_OF_MEMORY;
  }

  slash = (__extension__ (__builtin_constant_p ('/') && !__builtin_constant_p (req->share) && ('/') == '\0' ? (char *) __rawmemchr (req->share, '/') : __builtin_strchr (req->share, '/')));
  if(!slash)
    slash = (__extension__ (__builtin_constant_p ('\\') && !__builtin_constant_p (req->share) && ('\\') == '\0' ? (char *) __rawmemchr (req->share, '\\') : __builtin_strchr (req->share, '\\')));

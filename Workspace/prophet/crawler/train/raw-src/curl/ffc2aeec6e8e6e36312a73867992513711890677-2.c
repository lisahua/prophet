                     ...
                     size_t length)
{
  ...
  if(data->set.verbose) {
    ...
    case 31:
      if(length > 4)
        Curl_infof(data, "Width: %hu ; Height: %hu", (pointer[1]<<8) | pointer[2],
...
# 926 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/telnet.c"
...
# 1089 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/telnet.c"
  while(count--) {
    ...
    case CURL_TS_CR:
      tn->telrcv_state = CURL_TS_DATA;
      if(c == '\0') {
        if(startwrite >= 0) { result = Curl_client_write(conn, (1<<0), (char *)&inbuf[startwrite], in-startwrite); if(result) return result; } startwrite = -1;
        break;
      }
  ...
  while(!result && nread--) {
    ...
    do {
      ...
      switch (Curl_poll(pfd, 1, -1)) {
        ...
        default:
          bytes_written = 0;
          result = Curl_write(conn, conn->sock[0],
...
# 1304 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/telnet.c"
  int interval_ms;
  struct pollfd pfd[2];
  int poll_cnt;
  ...
  *done = 1;
  ...
  if(result)
    return result;
# 1568 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/lib/telnet.c"
  pfd[0].fd = sockfd;
  pfd[0].events = 0x001;
  ...
  while(keepon) {
    ...
    default:
      if(pfd[0].revents & 0x001) {
        ...
        result = Curl_read(conn, sockfd, buf, 16384 - 1, &nread);

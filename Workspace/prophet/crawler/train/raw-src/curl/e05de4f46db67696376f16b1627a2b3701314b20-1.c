# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
# 22 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c"
...
# 23 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
...
# 25 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
...
# 59 "../lib/curlx.h" 2
# 29 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
...
# 31 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
# 1 "../src/tool_convert.h" 1
# 32 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
...
# 33 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2


# 1 "../lib/memdebug.h" 1
# 36 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c" 2
...
void clean_getout(struct OperationConfig *config)
{
  ...
  while(node) {
    next = node->next;
    do {if((node->url)) {free((node->url)); (node->url) = ((void*)0);}} while(0);
    do {if((node->outfile)) {free((node->outfile)); (node->outfile) = ((void*)0);}} while(0);
    do {if((node->infile)) {free((node->infile)); (node->infile) = ((void*)0);}} while(0);
    do {if((node)) {free((node)); (node) = ((void*)0);}} while(0);
    node = next;
  }
...
_Bool output_expected(const char *url, const char *uploadfile)
{
  if(!uploadfile)
    return 1;
  if(Curl_raw_nequal("http://",url,strlen("http://")) || Curl_raw_nequal("https://",url,strlen("https://")))
    return 1;
...
_Bool stdin_upload(const char *uploadfile)
{
  return (curl_strequal(uploadfile, "-") ||
          curl_strequal(uploadfile, ".")) ? 1 : 0;
}
...
char *add_file_name_to_url(CURL *curl, char *url, const char *filename)
{
  ...
  if(!ptr || !strlen(++ptr)) {
    ...
    if(encfile) {
      ...
      curl_free(encfile);
      do {if((url)) {free((url)); (url) = ((void*)0);}} while(0);
...
CURLcode get_url_file_name(char **filename, const char *url)
{
  ...
  *filename = (__extension__ (__builtin_constant_p (pc) && ((size_t)(const void *)((pc) + 1) - (size_t)(const void *)(pc) == 1) ? (((const char *) (pc))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (pc) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, pc, __len); __retval; })) : __strdup (pc)));
  if(!*filename)
    return CURLE_OUT_OF_MEMORY;
# 171 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/src/tool_operhlp.c"
  return CURLE_OK;
}

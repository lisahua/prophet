...
# 74 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c" 2


# 1 "../lib/memdebug.h" 1
# 77 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c" 2
...
void Curl_free_ssl_config(struct ssl_config_data* sslc)
{
  do { Curl_cfree((sslc->CAfile)); (sslc->CAfile) = ((void*)0);} while(0);
  do { Curl_cfree((sslc->CApath)); (sslc->CApath) = ((void*)0);} while(0);
  do { Curl_cfree((sslc->cipher_list)); (sslc->cipher_list) = ((void*)0);} while(0);
  do { Curl_cfree((sslc->egdsocket)); (sslc->egdsocket) = ((void*)0);} while(0);
  do { Curl_cfree((sslc->random_file)); (sslc->random_file) = ((void*)0);} while(0);
}
# 184 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c"
unsigned int Curl_rand(struct SessionHandle *data)
{
  unsigned int r = 0;
  static unsigned int randseed;
  static _Bool seeded = 0;
# 207 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c"
...
void Curl_ssl_kill_session(struct curl_ssl_session *session)
{
  if(session->sessionid) {
    ...
    do { Curl_cfree((session->name)); (session->name) = ((void*)0);} while(0);
  }
}
...
void Curl_ssl_close_all(struct SessionHandle *data)
{
  ...
  if(data->state.session && !(data->share && (data->share->specifier & (1<<CURL_LOCK_DATA_SSL_SESSION)))) {
    ...
    do { Curl_cfree((data->state.session)); (data->state.session) = ((void*)0);} while(0);
  }
...
# 578 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c"
int Curl_ssl_check_cxn(struct connectdata *conn)
{
                                  ...
                                  unsigned char **der, size_t *der_len)
{
  ...
  do { Curl_cfree((stripped_pem)); (stripped_pem) = ((void*)0);} while(0);
                              ...
                              const unsigned char *pubkey, size_t pubkeylen)
{
  ...
  do { Curl_cfree((buf)); (buf) = ((void*)0);} while(0);
  do { Curl_cfree((pem_ptr)); (pem_ptr) = ((void*)0);} while(0);
  fclose(fp);
...
# 846 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src1/lib/vtls/vtls.c"
}
...
_Bool Curl_ssl_cert_status_request(void)
{
  ...
  return Curl_ossl_cert_status_request();

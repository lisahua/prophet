# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c"
# 1 "<built-in>" 1
# 1 "<built-in>" 3
# 312 "<built-in>" 3
# 1 "<command line>" 1
# 1 "<built-in>" 2
# 1 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
# 22 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c"
...
# 23 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
...
# 26 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
...
# 28 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
# 1 "../src/tool_vms.h" 1
# 29 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2

# 1 "../lib/memdebug.h" 1
# 31 "/home/ubuntu/Workspace/prophet/crawler/__tmp/src2/src/tool_urlglob.c" 2
...
static CURLcode glob_fixed(URLGlob *glob, char *fixed, size_t len)
{
  ...
  if(!pat->content.Set.elements)
    return glob->error = "out of memory", glob->pos = 0, CURLE_OUT_OF_MEMORY;
                         ...
                         int globindex)
{
  ...
  while(!done) {
    switch (*pattern) {
    case '\0':
      return glob->error = "unmatched brace", glob->pos = opos, CURLE_URL_MALFORMAT;

    case '{':
    case '[':
      return glob->error = "nested brace", glob->pos = *posp, CURLE_URL_MALFORMAT;

    case '}':
      if(opattern == pattern)
        return glob->error = "empty string within braces", glob->pos = *posp, CURLE_URL_MALFORMAT;



      if(multiply(amount, pat->content.Set.size+1))
        return glob->error = "range overflow", glob->pos = 0, CURLE_URL_MALFORMAT;
    ...
    case ',':
      ...
      *buf = '\0';
      ...
      pat->content.Set.elements[pat->content.Set.size] =
        (__extension__ (__builtin_constant_p (glob->glob_buffer) && ((size_t)(const void *)((glob->glob_buffer) + 1) - (size_t)(const void *)(glob->glob_buffer) == 1) ? (((const char *) (glob->glob_buffer))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (glob->glob_buffer) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, glob->glob_buffer, __len); __retval; })) : __strdup (glob->glob_buffer)));
    ...
    case ']':
      return glob->error = "unexpected close bracket", glob->pos = *posp, CURLE_URL_MALFORMAT;
                           ...
                           int globindex)
{
  ...
  if((((*__ctype_b_loc ())[(int) (((int) ((unsigned char)*pattern)))] & (unsigned short int) _ISalpha))) {
    ...
    *posp += (pattern - *patternp);
    ...
    if((rc != 2) || (min_c >= max_c) || ((max_c - min_c) > ('z' - 'a')) ||
      ...
      return glob->error = "bad range", glob->pos = *posp, CURLE_URL_MALFORMAT;
    ...
    if(multiply(amount, (pat->content.CharRange.max_c -
      ...
      return glob->error = "range overflow", glob->pos = *posp, CURLE_URL_MALFORMAT;
  }
  else if((((*__ctype_b_loc ())[(int) (((int) ((unsigned char)*pattern)))] & (unsigned short int) _ISdigit))) {
    ...
    if(*pattern == '0') {
      ...
      c = pattern;
      while((((*__ctype_b_loc ())[(int) (((int) ((unsigned char)*c)))] & (unsigned short int) _ISdigit))) {
        c++;
    ...
    if((*__errno_location ()) || (endp == pattern))
      endp=((void*)0);
    else {
      if(*endp != '-')
        endp = ((void*)0);
    ...
    *posp += (pattern - *patternp);
    ...
    return glob->error = "bad range specification", glob->pos = *posp, CURLE_URL_MALFORMAT;

  *patternp = pattern;
  return CURLE_OK;
}

static _Bool peek_ipv6(const char *str, size_t *skip)
{
  ...
  for(;;) {
    const char c = str[i++];
    if((((*__ctype_b_loc ())[(int) (((int) ((unsigned char)c)))] & (unsigned short int) _ISalnum)) || c == '.' || c == '%') {
  ...
  *amount = 1;
  ...
  while(*pattern && !res) {
    ...
    while(*pattern && *pattern != '{') {
      ...
      if(*pattern == '}' || *pattern == ']')
        return glob->error = "unmatched close brace/bracket", glob->pos = pos, CURLE_URL_MALFORMAT;
    ...
    if(++glob->size > 100)
      return glob->error = "too many globs", glob->pos = pos, CURLE_URL_MALFORMAT;
  }
  return res;
}
                  ...
                  FILE *error)
{
  ...
  *glob = ((void*)0);
  ...
  if(!glob_expand) {
    do {if((glob_buffer)) {free((glob_buffer)); (glob_buffer) = ((void*)0);}} while(0);
    return CURLE_OUT_OF_MEMORY;
  }
  ...
  if(!res)
    *urlnum = amount;
    ...
    if(error && glob_expand->error) {
      ...
      const char *t;
      if(glob_expand->pos) {
        curl_msnprintf(text, sizeof(text), "%s in column %zu", glob_expand->error,
                 glob_expand->pos);
        t = text;
      }
      else
      ...
      curl_mfprintf(error, "curl: (%d) [globbing] %s\n", res, t);
    }
  ...
  for(i = 0; i < glob->size; i++) {
    if((glob->pattern[i].type == UPTSet) &&
      ...
      for(elem = glob->pattern[i].content.Set.size - 1;
          elem >= 0;
          --elem) {
        do {if((glob->pattern[i].content.Set.elements[elem])) {free((glob->pattern[i].content.Set.elements[elem])); (glob->pattern[i].content.Set.elements[elem]) = ((void*)0);}} while(0);
      }
      do {if((glob->pattern[i].content.Set.elements)) {free((glob->pattern[i].content.Set.elements)); (glob->pattern[i].content.Set.elements) = ((void*)0);}} while(0);
    }
  }
  do {if((glob->glob_buffer)) {free((glob->glob_buffer)); (glob->glob_buffer) = ((void*)0);}} while(0);
  do {if((glob)) {free((glob)); (glob) = ((void*)0);}} while(0);
}
...
CURLcode glob_next_url(char **globbed, URLGlob *glob)
{
  ...
  for(i = 0; i < glob->size; ++i) {
    ...
    case UPTCharRange:
      *buf++ = pat->content.CharRange.ptr_c;
      break;
    case UPTNumRange:
      len = curl_msnprintf(buf, buflen, "%0*ld",
                     pat->content.NumRange.padlength,
  ...
  *globbed = (__extension__ (__builtin_constant_p (glob->glob_buffer) && ((size_t)(const void *)((glob->glob_buffer) + 1) - (size_t)(const void *)(glob->glob_buffer) == 1) ? (((const char *) (glob->glob_buffer))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (glob->glob_buffer) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void*)0)) __retval = (char *) memcpy (__retval, glob->glob_buffer, __len); __retval; })) : __strdup (glob->glob_buffer)));
  if(!*globbed)
    return CURLE_OUT_OF_MEMORY;
...
CURLcode glob_match_url(char **result, char *filename, URLGlob *glob)
{
  ...
  *result = ((void*)0);
  ...
  while(*filename) {
    if(*filename == '#' && (((*__ctype_b_loc ())[(int) (((int) ((unsigned char)filename[1])))] & (unsigned short int) _ISdigit))) {
      unsigned long i;
      char *ptr = filename;
      ...
      if(pat) {
        ...
        case UPTCharRange:
          ...
          break;
        case UPTNumRange:
          curl_msnprintf(numbuf, sizeof(numbuf), "%0*d",
                   pat->content.NumRange.padlength,
                   pat->content.NumRange.ptr_n);
          appendthis = numbuf;
        ...
        default:
          curl_mfprintf(stderr, "internal error: invalid pattern type (%d)\n",
                  (int)pat->type);
          do {if((target)) {free((target)); (target) = ((void*)0);}} while(0);
          return CURLE_FAILED_INIT;
        }
      }
      else {
